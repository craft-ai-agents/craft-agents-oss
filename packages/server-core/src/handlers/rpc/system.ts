...
import type { GitStatusResult, GitStatusFileEntry, GitFileDiffResult } from '@archstudio/shared/protocol'
import { getWorkspaceByNameOrId, getGitBashPath, setGitBashPath, clearGitBashPath, CONFIG_DIR } from '@archstudio/shared/config'
...
function parseInternalARCHstudioDeepLink(parsed: URL): ParsedInternalDeepLink | null {
  if (parsed.protocol !== 'archstudio:') return null
  ...
  server.handle(RPC_CHANNELS.shell.OPEN_URL, async (ctx, url: string) => {
    ...
    try {
      ...
      if (classification.kind === 'internal-deeplink') {
        const deepLink = parseInternalARCHstudioDeepLink(parsed)
        ...
        if (deepLink?.navigation?.view || deepLink?.navigation?.action) {
          ...
          deps.platform.logger.info('[OPEN_URL] Routing archstudio:// URL internally via deeplink:navigate')
        ...
        deps.platform.logger.info('[OPEN_URL] Falling back to client openExternal for archstudio:// URL')
