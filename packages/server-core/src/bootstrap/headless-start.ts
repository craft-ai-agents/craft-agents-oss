...
import { ensureConfigDir, loadStoredConfig, saveConfig } from '@archstudio/shared/config'
        ...
        if (lock.pid === process.pid) {
          ...
          if (isLockFromPreviousBoot(lock.startedAt)) {
            ...
            throw new Error(
              ...
              `To run a parallel instance (e.g. for dev), set ARCHSTUDIO_CONFIG_DIR to a different path.`
            )
          }
        } else {
...
export async function bootstrapServer<TSessionManager, THandlerDeps>(
  ...
  const serverToken = options.serverToken ?? process.env.ARCHSTUDIO_SERVER_TOKEN
  if (!serverToken) {
    throw new Error('Server token is required. Pass options.serverToken or set ARCHSTUDIO_SERVER_TOKEN.')
  }
  ...
  const bundledAssetsRoot = options.bundledAssetsRoot
    ?? process.env.ARCHSTUDIO_BUNDLED_ASSETS_ROOT
    ?? process.cwd()
  ...
  const rpcHost = options.rpcHost ?? process.env.ARCHSTUDIO_RPC_HOST ?? '127.0.0.1'
  const rpcPortRaw = options.rpcPort ?? parseInt(process.env.ARCHSTUDIO_RPC_PORT ?? '9100', 10)
  ...
  platform.logger.info(`ARCHstudio server listening on ${wsServer.protocol}://${rpcHost}:${wsServer.port}`)
