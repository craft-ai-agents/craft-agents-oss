...
import type { StoredAttachment, ContentBadge } from '@g4os/core'
import { normalizePath } from '@g4os/core/utils'
...
function InlineFileBadge({
  ...
  // Strip .g4os workspace/session path prefix for cleaner tooltip display
  // e.g. "/Users/.../workspaces/{id}/sessions/{id}/plans/foo.md" → "plans/foo.md"
  const rawPath = badge.filePath || badge.label
  const tooltipPath = normalizePath(rawPath).replace(/^.*\.g4os\/workspaces\/[^/]+\/(sessions\/[^/]+\/)?/, '')
