...
export interface ResolveScriptRuntimeContext {
  /**
   * Whether host app is packaged. Defaults to ARCHSTUDIO_IS_PACKAGED=1.
...
function inferPackagedMode(ctx?: ResolveScriptRuntimeContext): boolean {
  ...
  return process.env.ARCHSTUDIO_IS_PACKAGED === '1';
}
...
function resolveResourcesBase(ctx?: ResolveScriptRuntimeContext): string | null {
  const explicit = ctx?.resourcesBasePath || process.env.ARCHSTUDIO_RESOURCES_BASE;
...
function resolveAppRoot(ctx?: ResolveScriptRuntimeContext): string | null {
  const explicit = ctx?.appRootPath || process.env.ARCHSTUDIO_APP_ROOT;
...
function validatePackagedEnvRuntime(command: string, label: string): string {
  ...
  if (!isAbsolute(command) && !hasPathSeparator) {
    throw new Error(
      ...
      'Packaged builds do not allow PATH-based runtime resolution. Configure an absolute ARCHSTUDIO_* path or ship a bundled runtime.'
    );
  }
  ...
  if (!existsSync(resolvedCommand)) {
    throw new Error(
      ...
      'Configure a valid absolute ARCHSTUDIO_* path or ship a bundled runtime.'
    );
  }
 ...
 *
 * Resolution order:
 * - env override (ARCHSTUDIO_UV / ARCHSTUDIO_NODE / ARCHSTUDIO_BUN)
...
export function resolveScriptRuntime(
  ...
  if (language === 'python3') {
    if (process.env.ARCHSTUDIO_UV) {
      const cmd = isPackaged
        ? validatePackagedEnvRuntime(process.env.ARCHSTUDIO_UV, 'Python/uv')
        : process.env.ARCHSTUDIO_UV;
    ...
    throw new Error(
      isPackaged
        ...
        : 'Python runtime unavailable: uv was not found. Configure ARCHSTUDIO_UV or install uv on PATH.'
    );
  }

  if (language === 'node') {
    if (process.env.ARCHSTUDIO_NODE) {
      const cmd = isPackaged
        ? validatePackagedEnvRuntime(process.env.ARCHSTUDIO_NODE, 'Node')
        : process.env.ARCHSTUDIO_NODE;
    ...
    throw new Error(
      isPackaged
        ...
        : 'Node runtime unavailable: configure ARCHSTUDIO_NODE or install node on PATH.'
    );
  }

  if (process.env.ARCHSTUDIO_BUN) {
    const cmd = isPackaged
      ? validatePackagedEnvRuntime(process.env.ARCHSTUDIO_BUN, 'Bun')
      : process.env.ARCHSTUDIO_BUN;
    return { command: cmd, argsPrefix: [], source: 'env' };
  }
  ...
  throw new Error(
    isPackaged
      ...
      : 'Bun runtime unavailable: configure ARCHSTUDIO_BUN or install bun on PATH.'
  );
}
