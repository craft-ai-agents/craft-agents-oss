...
`@g4os/shared` is the core business logic package for G4 OS. It contains:
- Agent implementation (G4Agent, session-scoped tools, permission modes)
...
```typescript
import { G4Agent, getPermissionMode, setPermissionMode } from '@g4os/shared/agent';
import { loadStoredConfig, type Workspace } from '@g4os/shared/config';
import { getCredentialManager } from '@g4os/shared/credentials';
import { G4OSMcpClient } from '@g4os/shared/mcp';
import { loadWorkspaceSources, type LoadedSource } from '@g4os/shared/sources';
import { loadStatusConfig, createStatus } from '@g4os/shared/statuses';
import { resolveTheme } from '@g4os/shared/config/theme';
...
```
src/
├── agent/              # G4Agent, session-scoped-tools, mode-manager, mode-types, permissions-config
...
### G4Agent (`src/agent/claude-agent.ts`)
...
- Workspace: `~/.g4os/workspaces/{id}/permissions.json`
- Source: `~/.g4os/workspaces/{id}/sources/{slug}/permissions.json`
...
**Storage:** `~/.g4os/workspaces/{id}/statuses/config.json`
...
**Storage:**
- App: `~/.g4os/theme.json`
- Workspace: `~/.g4os/workspaces/{id}/theme.json`
...
All sensitive credentials (API keys, OAuth tokens) are stored in an AES-256-GCM encrypted file at `~/.g4os/credentials.enc`. The `CredentialManager` provides the API for reading and writing credentials.
...
│     └── ~/.g4os/workspaces/{ws}/sources/{slug}/          │
└─────────────────────────────────────────────────────────────────┘
                              │
...
Multi-workspace configuration stored in `~/.g4os/config.json`. Supports:
...
### Sources (`src/sources/`)
Sources are external data connections (MCP servers, APIs, local filesystems). Stored at `~/.g4os/workspaces/{id}/sources/{slug}/` with config.json and guide.md. Types: `mcp`, `api`, `local`, `gmail`.

## Dependencies

- `@g4os/core` - Shared types
