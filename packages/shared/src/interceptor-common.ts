...
export const CONFIG_FILE = join(homedir(), '.mdp-agent', 'config.json');
...
export const LOG_DIR = join(homedir(), '.mdp-agent', 'logs');
...
function getErrorFilePath(): string {
  ...
  return join(homedir(), '.mdp-agent', 'api-error.json');
}
