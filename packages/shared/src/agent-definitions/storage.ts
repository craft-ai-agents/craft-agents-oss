...
export function getGlobalAgentDir(slug: string): string {
  return join(GLOBAL_AGENTS_DIR, slug);
}

/** Get the absolute path to an agent's AGENT.md file in the global library. */
export function getGlobalAgentFile(slug: string): string {
  return join(getGlobalAgentDir(slug), AGENT_FILE);
}
...
function coercePermissionMode(value: unknown): PermissionMode | undefined {
 ...
 */
export function parseAgentFile(content: string): { metadata: AgentMetadata; systemPrompt: string } | null {
  ...
  const metadata: AgentMetadata = {
    ...
    permissionMode: coercePermissionMode(data.permissionMode),
    thinkingLevel: coerceThinkingLevel(data.thinkingLevel),
    skills: coerceStringArray(data.skills),
    sources: coerceStringArray(data.sources),
...
export function loadAllGlobalAgents(): LoadedAgent[] {
  if (!existsSync(GLOBAL_AGENTS_DIR)) return [];

  const out: LoadedAgent[] = [];
  let entries: import('node:fs').Dirent[];
  try {
    entries = readdirSync(GLOBAL_AGENTS_DIR, { withFileTypes: true });
  } catch {
    return [];
  }
  for (const entry of entries) {
    ...
    const agent = loadAgentFromDir(join(GLOBAL_AGENTS_DIR, entry.name), entry.name, 'global');
...
export function loadGlobalAgent(slug: string): LoadedAgent | null {
  if (!isValidAgentSlug(slug)) return null;
  return loadAgentFromDir(getGlobalAgentDir(slug), slug, 'global');
}
 ...
 */
export function loadActivatedAgents(workspaceRootPath: string): LoadedAgent[] {
 ...
 */
export function deleteGlobalAgent(slug: string, workspaceRootPaths: string[]): boolean {
  if (!isValidAgentSlug(slug)) return false;
  const dir = getGlobalAgentDir(slug);
 ...
 */
export function seedGlobalLibraryIfEmpty(starters: ReadonlyArray<{ slug: string; metadata: AgentMetadata; systemPrompt: string }>): { seeded: number } {
  mkdirSync(GLOBAL_AGENTS_DIR, { recursive: true });
  const marker = join(GLOBAL_AGENTS_DIR, '.seeded');
  if (existsSync(marker)) return { seeded: 0 };

  let seeded = 0;
  for (const starter of starters) {
    if (!isValidAgentSlug(starter.slug)) continue;
    const dir = getGlobalAgentDir(starter.slug);
    const file = join(dir, AGENT_FILE);
