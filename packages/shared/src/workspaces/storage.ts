 ...
 * Default location: ~/.g4os/workspaces/
 */
...
/**
 * Get the default workspaces directory (~/.g4os/workspaces/)
 */
 ...
 * @param baseDir - Parent directory where workspace folders live (e.g., ~/.g4os/workspaces/)
 ...
 * Returns paths to valid workspaces found in ~/.g4os/workspaces/
 */
 ...
 * Resolution order: ENV (G4OS_LOCAL_MCP_ENABLED) > workspace config > default (true)
 *
 * @param rootPath - Absolute path to workspace root folder
...
export function isLocalMcpEnabled(rootPath: string): boolean {
  ...
  const envValue = process.env.G4OS_LOCAL_MCP_ENABLED;
