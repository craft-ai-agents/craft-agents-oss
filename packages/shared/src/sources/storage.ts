 ...
 * @param workspaceRootPath - Absolute path to workspace folder (e.g., ~/.g4os/workspaces/xxx)
 ...
 * (like g4os-docs) that don't have filesystem folders.
 */
export function getSourcesBySlugs(workspaceRootPath: string, slugs: string[]): LoadedSource[] {
  ...
  for (const slug of slugs) {
    ...
    if (isBuiltinSource(slug)) {
      // Currently only g4os-docs is a builtin source
      if (slug === 'g4os-docs') {
 ...
 * Built-in sources (like g4os-docs) are always available and merged
...
export async function createSource(
  ...
  // (bundled guides removed - agent should search g4os-docs MCP for service-specific guidance)
