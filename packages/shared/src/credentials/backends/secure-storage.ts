 ...
 *
 * Stores credentials in an encrypted file at ~/.g4os/credentials.enc
...
// File location
const CREDENTIALS_DIR = join(homedir(), '.g4os');
...
export class SecureStorageBackend implements CredentialBackend {
  ...
  private getEncryptionKey(salt: Buffer): Buffer {
    ...
    const stableMachineId = createHash('sha256')
      ...
      .update('g4os-v2') // Bumped version for new key derivation
      .digest();
