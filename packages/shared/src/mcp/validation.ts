 ...
 *
 * Validates HTTP/SSE MCP servers directly via CraftMcpClient instead of
 * spawning a Claude subprocess, which is fragile inside Electron sandboxing.
 */
...
export interface McpValidationConfig {
  ...
  /** @deprecated HTTP MCP validation no longer uses Claude credentials. */
  claudeApiKey?: string;
  /** @deprecated HTTP MCP validation no longer uses Claude credentials. */
  claudeOAuthToken?: string;
  /** @deprecated HTTP MCP validation no longer uses a model. */
  model?: string;
}
 ...
 * listing tools. The internal connect path performs a listTools health check.
 */
