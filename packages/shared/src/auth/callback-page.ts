...
<body>
  <div class="content">
    ...
    ${deeplinkUrl ? `<a href="${deeplinkUrl}" class="return-link">MDP</a>` : ''}
  </div>
...
/**
 * Generate the page served by the SSO relay after the OIDC provider redirects
 * back. Success hands off to the Electron deep link; errors stay in-browser.
 */
export function generateSsoRelayCallbackPage(deepLinkUrl: string, errorDetail?: string): string {
  const isSuccess = errorDetail === undefined;
  const statusMessage = isSuccess
    ? 'Login successful'
    : `Login failed: ${escapeHtml(errorDetail)}`;
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Craft - ${isSuccess ? 'Login Successful' : 'Login Failed'}</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
...
<body>
  <div class="content">
    ...
    ${isSuccess ? `<a href="${escapedDeepLinkUrl}" class="return-link">Open MDP</a>` : ''}
    <div class="hint">${isSuccess ? 'You can close this tab.' : 'Close this window and try again.'}</div>
  </div>
  <script>${autoRedirectScript}</script>
</body>
</html>`;
}
