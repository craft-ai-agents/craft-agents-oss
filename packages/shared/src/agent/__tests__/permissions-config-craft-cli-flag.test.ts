...
const originalCliFlag = process.env.ARCHSTUDIO_FEATURE_ARCHSTUDIO_AGENTS_CLI
    ...
    JSON.stringify(
      {
        version: '2026-03-07',
        allowedBashPatterns: [
          { pattern: '^archstudio\\s+label\\s+list\\b', comment: 'archstudio label read-only operations' },
...
afterEach(() => {
  ...
  if (originalCliFlag === undefined) delete process.env.ARCHSTUDIO_FEATURE_ARCHSTUDIO_AGENTS_CLI
  else process.env.ARCHSTUDIO_FEATURE_ARCHSTUDIO_AGENTS_CLI = originalCliFlag
})
...
describe('permissions config craft-agents-cli feature flag', () => {
  it('skips compiling craft-agent bash allowlist patterns when feature is disabled', () => {
    ...
    try {
      process.env.ARCHSTUDIO_CONFIG_DIR = tempConfigDir
      ...
      expect(sources.some(source => source.startsWith('^archstudio\\s'))).toBe(false)
  ...
  it('compiles craft-agent bash allowlist patterns when feature is enabled', () => {
    ...
    try {
      ...
      const sources = merged.readOnlyBashPatterns.map(p => p.source)
      expect(sources).toContain('^archstudio\\s+label\\s+list\\b')
      expect(sources).toContain('^rg\\b')
    } finally {
      rmSync(tempConfigDir, { recursive: true, force: true })
    }
