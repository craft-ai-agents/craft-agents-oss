#!/usr/bin/env bun
/**
 * Debug script to print the full G4 OS system prompt with annotations.
...
printAnnotation('  2. G4 OS Environment Marker - version, platform, arch');
...
const systemPrompt = getSystemPrompt(
  ...
  '/Users/example/.g4os/workspaces/abc123' // Example workspace path
);
...
const systemPromptWithDebug = getSystemPrompt(
  undefined,
  { enabled: true, logFilePath: '~/Library/Logs/@g4os/electron/main.log' },
  '/Users/example/.g4os/workspaces/abc123'
);
...
const sessionState = formatSessionState('260121-example-session', {
  plansFolderPath:
    '/Users/example/.g4os/workspaces/abc123/sessions/260121-example-session/plans',
});
...
printAnnotation('Generated by G4Agent.formatSourceState() - requires agent instance');
...
${colors.bold}SDK Configuration:${colors.reset}
  ...
  systemPrompt.append: getSystemPrompt() ${colors.dim}// G4 OS additions (static, cacheable)${colors.reset}
...
${colors.bold}Static System Prompt Components:${colors.reset}
  ...
  2. G4 OS Environment Marker      ${colors.dim}// Version, platform, arch${colors.reset}
