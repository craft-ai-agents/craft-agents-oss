#!/usr/bin/env bun
/**
 * Debug script to print the full ARCHstudio system prompt with annotations.
...
printAnnotation('  2. ARCHstudio Environment Marker - version, platform, arch');
...
const systemPrompt = getSystemPrompt(
  ...
  '/Users/example/.archstudio/workspaces/abc123' // Example workspace path
);
...
const systemPromptWithDebug = getSystemPrompt(
  undefined,
  { enabled: true, logFilePath: '~/Library/Logs/@archstudio/electron/main.log' },
  '/Users/example/.archstudio/workspaces/abc123'
);
...
const sessionState = formatSessionState('260121-example-session', {
  plansFolderPath:
    '/Users/example/.archstudio/workspaces/abc123/sessions/260121-example-session/plans',
});
...
${colors.bold}SDK Configuration:${colors.reset}
  ...
  systemPrompt.append: getSystemPrompt() ${colors.dim}// ARCHstudio additions (static, cacheable)${colors.reset}
...
${colors.bold}Static System Prompt Components:${colors.reset}
  ...
  2. ARCHstudio Environment Marker      ${colors.dim}// Version, platform, arch${colors.reset}
