...
import { migrateAutogeneratedProviderName } from './connection-names.ts';
...
function migrateProviderConnectionNames(config: StoredConfig): boolean {
  if (!config.llmConnections) return false;
  let changed = false;
  for (const connection of config.llmConnections) {
    const migratedName = migrateAutogeneratedProviderName(connection);
    if (migratedName && migratedName !== connection.name) {
      connection.name = migratedName;
      changed = true;
    }
  }
  return changed;
...
export function migrateLegacyLlmConnectionsConfig(): void {
  ...
  if (config.llmConnections !== undefined) {
    ...
    // Phase 1l: Replace untouched backend-oriented labels with provider names.
    if (migrateProviderConnectionNames(config)) {
      needsSave = true;
    }
  ...
  migrateProviderConnectionNames(config);
  migrateWorkspaceLegacyOpusToDefaultOpus(config);
