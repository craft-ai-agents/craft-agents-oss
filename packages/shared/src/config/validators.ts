  ...
  try {
    ...
    return {
      ...
      errors: [{
        ...
        severity: 'error',
        suggestion: 'See ~/.g4os/docs/skills.md for SKILL.md format reference',
      }],
      warnings: [],
    };
  }
        ...
        if (tool.icon) {
          ...
          if (!existsSync(iconPath)) {
            result.warnings.push({
              ...
              severity: 'warning',
              suggestion: `Place '${tool.icon}' in ~/.g4os/tool-icons/`,
            });
          }
        }
      }
 ...
 * Checks paths relative to CONFIG_DIR (~/.g4os/).
 * Returns null if the path is not a recognized app-level config file.
 *
 * Matches patterns:
 * - ~/.g4os/tool-icons/tool-icons.json → tool icon mappings
 */
