...
**文本框永远在、永不被替换。** 任务按钮是输入框**上方**的一排快捷入口。这把整个交互简化成"输入框上方多了一个组件",不碰 `InputContainer` 的 freeform/structured 切换逻辑,也不需要"逃生口"——文本框始终可用就是天然逃生口。
...
```
按钮排常驻在输入框上方
  ├─ 点某任务按钮 → 该任务表单在按钮排与输入框之间【内联展开】
  │    ├─ 填好 → 提交 → toMessage() → onSubmit(text) → 表单收起
  │    ├─ 再点同一按钮 / 点 ✕ → 表单收起
  │    └─ 点另一按钮 → 切换到那个任务的表单
...
- 按钮排**常驻**(空会话和会话中都在);表单面板**仅选中某任务时出现**,默认收起,只占一行按钮高度。
- 一次只展开一个任务表单(单选语义,再点收起 / 切换)。
...
| `app-shell/input/TaskBar.tsx`(新) | 按钮排 + 内联表单面板 + 通用字段渲染器;提交调用透传进来的 `onSubmit` |
