...
Terminal client for MDP server. Connects over WebSocket (`ws://` or `wss://`) to a running headless server.
...
- For all other commands: a running MDP headless server with URL and token
