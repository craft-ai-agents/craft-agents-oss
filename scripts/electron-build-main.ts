...
const WA_WORKER_BUILD_SCRIPT = join(ROOT_DIR, "scripts/build-wa-worker.ts");
...
async function buildWhatsAppWorker(): Promise<void> {
  if (!existsSync(join(WA_WORKER_DIR, "src/worker.ts")) || !existsSync(WA_WORKER_BUILD_SCRIPT)) {
  ...
  const proc = spawn({
    cmd: ["bun", "run", WA_WORKER_BUILD_SCRIPT],
    cwd: ROOT_DIR,
    stdout: "inherit",
    stderr: "inherit",
  });
