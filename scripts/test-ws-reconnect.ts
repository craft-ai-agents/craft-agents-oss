 ...
 *
 * Usage:
 *   bun scripts/test-ws-reconnect.ts <host> <port> <token> [--kill-after=N]
 *
 * Example:
  ...
  return new Promise((resolve, reject) => {
    ...
    socket.on('message', (raw) => {
      ...
      if (envelope.type === 'event') {
        ...
        eventCount++
        logEvent(seq, envelope.channel, eventType)
...
async function triggerStreaming(socket: WebSocket): Promise<void> {
  ...
  const createResponse = await rpcInvoke(socket, 'sessions:create', createId, [
    { prompt: '' } // minimal session creation
  ])
  ...
  if (createResponse?.result) {
    ...
    const listResponse = await rpcInvoke(socket, 'sessions:get', listId, [])
  ...
  rpcInvoke(socket, 'sessions:send-message', sendId, [
    sessionId,
    {
      content: 'Count from 1 to 50 slowly, one number per line.',
      attachments: [],
    },
  ]).catch(() => {}) // fire and forget — we'll see events
