...
DOWNLOAD_DIR="$HOME/.mdp-agent/downloads"
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    if pgrep -x "MDP" >/dev/null 2>&1; then
        info "Quitting MDP..."
    ...
    echo ""
    printf "%b\n" "  MDP has been installed to ${BOLD}$INSTALL_DIR/$APP_NAME${NC}"
    echo ""
    ...
    # New paths
    APP_DIR="$HOME/.mdp-agent/app"
    WRAPPER_PATH="$INSTALL_DIR/craft-agents"
    APPIMAGE_INSTALL_PATH="$APP_DIR/MDP-x64.AppImage"

    # Kill the app if it's running
    if pgrep -f "MDP.*AppImage" >/dev/null 2>&1; then
        info "Stopping MDP..."
        pkill -f "MDP.*AppImage" 2>/dev/null || true
        sleep 2
    fi
...
#!/bin/bash
# MDP launcher - handles Linux-specific AppImage issues

APPIMAGE_PATH="$HOME/.mdp-agent/app/MDP-x64.AppImage"
ELECTRON_CACHE="$HOME/.config/@craft-agent"
ELECTRON_CACHE_ALT="$HOME/.cache/@craft-agent"

# Verify AppImage exists
if [ ! -f "$APPIMAGE_PATH" ]; then
    echo "Error: MDP not found at $APPIMAGE_PATH"
...
WRAPPER_EOF
    ...
    OLD_APPIMAGE="$INSTALL_DIR/MDP-x64.AppImage"
