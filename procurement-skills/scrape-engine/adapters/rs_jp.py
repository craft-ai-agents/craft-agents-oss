#!/usr/bin/env python3
"""GENERIC SSR-dump adapter 'rs-jp' — mode=dom, body-text -> Row.note.
Ported from legacy extended-source cloak_search.py
(scrape_generic + GENERIC table row 'rs-jp'). url_tpl + needs_proxy lifted as-is;
...
ADAPTER = make_generic(
    'rs-jp',
    'https://jp.rs-online.com/web/c/?searchTerm={q}',
    True,
    exclusive=True,
)
