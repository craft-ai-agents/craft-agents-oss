...
class BatchOrchestrationSkillContractTests(unittest.TestCase):
    ...
    def test_finding_scenario_recipe_preserves_completeness_requirements(self) -> None:
        text = self.finding_recipe_text
        self.assertIn("和单型号主流程同等完整", text)
        self.assertIn("先本地库存", text)
        self.assertIn("`--source-set direct`", text)
        self.assertIn("每个直接平台都有状态", text)
        self.assertIn("命令超时不是降级理由", text)
