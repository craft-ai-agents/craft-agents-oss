...
| 常见借口 | 反驳 |
|---|---|
| “业务表才是真表，直接写进去更方便。” | 只写 AI Base 的 `AI-` 表；业务 Base 是只读边界。 |
| “用户已经说写表了，dry-run 可以省。” | 必须先 dry-run 预览，确认后才正式写。 |
| “顺手去重/合并一下更干净。” | 写入是 append-only；不去重、不合并、不改历史。 |
| “权限报错后我换身份多试几次。” | 权限/登录错误要如实报告，不假装成功、不乱试写入目标。 |
