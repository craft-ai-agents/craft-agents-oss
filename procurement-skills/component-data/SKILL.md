...
**不像旧 browserdepot 要 submit/wait/results 三步——这里一条命令并发跑完全部源、直接返回。** 每源独立超时隔离,一个死源不拖累整批;全量一轮通常 1 分钟左右,慢源(海外站远程渲染)拖尾时最长约 2 分钟——**别因为过了一分钟没返回就中断重跑**。生产上批次实际由常驻 daemon(`component-data.service`,共享暖浏览器池)执行,多人同时调用会排队而不是挤爆内存;调用方式不变,并发高峰时等待稍长是正常的。
