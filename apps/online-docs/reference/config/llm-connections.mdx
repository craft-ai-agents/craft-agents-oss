...
| `models` | No | Explicit model list. Accepts strings (`"gpt-5.4"`) or objects with optional `contextWindow` override (see [Custom Context Window](#custom-context-window)). |
...
Custom endpoints default to a **128K context window** for all models. If your model supports a larger context, set `contextWindow` on the model object to enable proper auto-compaction behavior:
...
```json
{
  ...
  "models": [
    ...
    { "id": "gpt-5.2", "name": "GPT 5.2", "contextWindow": 272000 }
  ],
...
<Note>
Plain string model entries (e.g., `"gpt-5.4"`) continue to use the 128K default. Use object entries with `contextWindow` only when you need a different value.
</Note>
