...
Bedrock uses your existing AWS credential chain — `~/.aws/credentials`, `AWS_PROFILE`, IAM roles, SSO sessions, and environment variables all work. The subprocess inherits your shell's AWS credentials automatically. No credentials are stored in Craft Agents.

Set `awsRegion` to the region where you have Bedrock model access enabled (e.g., `us-east-1`, `us-west-2`, `eu-west-1`).

<Note>
The `iam_credentials` auth type is also supported at the backend level for programmatic credential injection, but there is currently no UI form for entering IAM credentials directly. Use `environment` auth for the recommended setup.
</Note>
