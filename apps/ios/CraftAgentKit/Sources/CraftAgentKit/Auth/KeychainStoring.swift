...
public actor InMemoryKeychainStore: KeychainStoring {
    ...
    public init() {}
    public func save(_ data: Data, forKey key: String) async throws { storage[key] = data }
    public func load(forKey key: String) async throws -> Data? { storage[key] }
    public func delete(forKey key: String) async throws { storage.removeValue(forKey: key) }
}
