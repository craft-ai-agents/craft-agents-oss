...
const api: ElectronAPI = {
  ...
  createSource: (workspaceId: string, config: Partial<import('@g4os/shared/sources').FolderSourceConfig>) =>
  ...
  onSourcesChanged: (callback: (sources: import('@g4os/shared/sources').LoadedSource[]) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, sources: import('@g4os/shared/sources').LoadedSource[]) => {
      callback(sources)
    }
  ...
  onSkillsChanged: (callback: (skills: import('@g4os/shared/skills').LoadedSkill[]) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, skills: import('@g4os/shared/skills').LoadedSkill[]) => {
      callback(skills)
    }
  ...
  onAppThemeChange: (callback: (theme: import('@g4os/shared/config').ThemeOverrides | null) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, theme: import('@g4os/shared/config').ThemeOverrides | null) => {
      callback(theme)
    }
  ...
  // Menu actions (for unified G4 OS menu)
