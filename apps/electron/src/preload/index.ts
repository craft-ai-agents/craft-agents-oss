...
const api: ElectronAPI = {
  ...
  // Deep link navigation listener (for external vesper:// URLs)
  ...
  createSource: (workspaceId: string, config: Partial<import('@vesper/shared/sources').FolderSourceConfig>) =>
  ...
  onSourcesChanged: (callback: (sources: import('@vesper/shared/sources').LoadedSource[]) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, sources: import('@vesper/shared/sources').LoadedSource[]) => {
      callback(sources)
    }
  ...
  onSkillsChanged: (callback: (skills: import('@vesper/shared/skills').LoadedSkill[]) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, skills: import('@vesper/shared/skills').LoadedSkill[]) => {
      callback(skills)
    }
  ...
  onAppThemeChange: (callback: (theme: import('@vesper/shared/config').ThemeOverrides | null) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, theme: import('@vesper/shared/config').ThemeOverrides | null) => {
      callback(theme)
    }
