...
// Core types are re-exported from @g4os/core
...
export { PERMISSION_MODE_CONFIG } from '@g4os/shared/agent/modes';

// Import thinking level types
import type { ThinkingLevel } from '@g4os/shared/agent/thinking-levels';
export type { ThinkingLevel };
export { THINKING_LEVELS, DEFAULT_THINKING_LEVEL } from '@g4os/shared/agent/thinking-levels';
...
import type { AuthState, SetupNeeds } from '@g4os/shared/auth/types';
import type { AuthType } from '@g4os/shared/config/types';
export type { AuthState, SetupNeeds, AuthType };

// Import and re-export credential health types
import type { CredentialHealthStatus, CredentialHealthIssue, CredentialHealthIssueType } from '@g4os/shared/credentials/types';
...
import type { LoadedSource, FolderSourceConfig, SourceConnectionStatus } from '@g4os/shared/sources/types';
...
import type { AuthRequest as SharedAuthRequest, CredentialInputMode as SharedCredentialInputMode, CredentialAuthRequest as SharedCredentialAuthRequest } from '@g4os/shared/agent';
...
export { generateMessageId } from '@g4os/core/types';
...
export type { PermissionRequest as BasePermissionRequest } from '@g4os/core/types';
import type { PermissionRequest as BasePermissionRequest } from '@g4os/core/types';
...
// CredentialInputMode is imported from @g4os/shared/agent above
...
import type { Message } from '@g4os/core/types';
...
export type SessionEvent =
  ...
  | { type: 'tool_start'; sessionId: string; toolName: string; toolUseId: string; toolInput: Record<string, unknown>; toolIntent?: string; toolDisplayName?: string; toolDisplayMeta?: import('@g4os/core').ToolDisplayMeta; turnId?: string; parentToolUseId?: string; timestamp?: number }
...
export interface SendMessageOptions {
  ...
  badges?: import('@g4os/core').ContentBadge[]
 ...
 * Uses SharedSessionMetadata from @g4os/shared (not core SessionMetadata)
 */
...
export const IPC_CHANNELS = {
  ...
  // Deep link navigation (main → renderer, for external g4os:// URLs)
  ...
  // Default permissions from ~/.g4os/permissions/default.json
...
import type { Workspace, SessionMetadata, StoredAttachment as StoredAttachmentType } from '@g4os/core/types';
...
export interface ElectronAPI {
  ...
  // Deep link navigation listener (for external g4os:// URLs)
  ...
  getSourcePermissionsConfig(workspaceId: string, sourceSlug: string): Promise<import('@g4os/shared/agent').PermissionsConfigFile | null>
  getWorkspacePermissionsConfig(workspaceId: string): Promise<import('@g4os/shared/agent').PermissionsConfigFile | null>
  getDefaultPermissionsConfig(): Promise<{ config: import('@g4os/shared/agent').PermissionsConfigFile | null; path: string }>
  ...
  listLabels(workspaceId: string): Promise<import('@g4os/shared/labels').LabelConfig[]>
  createLabel(workspaceId: string, input: import('@g4os/shared/labels').CreateLabelInput): Promise<import('@g4os/shared/labels').LabelConfig>
  ...
  listViews(workspaceId: string): Promise<import('@g4os/shared/views').ViewConfig[]>
  saveViews(workspaceId: string, views: import('@g4os/shared/views').ViewConfig[]): Promise<void>
