...
function createMockWebContents() {
  ...
  return {
    ...
    _emitRaw: (event: string, ...args: any[]) => {
      for (const cb of listeners[event] || []) cb(...args)
    },
  }
}
...
mock.module('electron', () => ({
  ...
  app: {
    getPath: mock((name: string) => `/tmp/${name}`),
  },
  ipcMain: {
...
describe('BrowserPaneManager', () => {
  ...
  it('destroys child popups when parent instance is destroyed', () => {
    ...
    instance.pageView.webContents._emitRaw('did-create-window', popupWindow, { url: 'https://accounts.google.com/signin' })
