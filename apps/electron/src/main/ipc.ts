...
export function registerIpcHandlers(sessionManager: SessionManager, windowManager: WindowManager): void {
  ...
  ipcMain.handle(IPC_CHANNELS.OPEN_SESSION_IN_NEW_WINDOW, async (_event, workspaceId: string, sessionId: string) => {
    ...
    const deepLink = `vesper://allChats/chat/${sessionId}`
  ...
  // Shell operations - open URL in external browser (or handle vesper:// internally)
  ipcMain.handle(IPC_CHANNELS.OPEN_URL, async (_event, url: string) => {
    ...
    try {
      ...
      // Handle vesper:// URLs internally via deep link handler
  ...
  ipcMain.handle(IPC_CHANNELS.WORKSPACE_SETTINGS_GET, async (_event, workspaceId: string) => {
    ...
    return {
      ...
      taskCoordinationEnabled: config?.defaults?.taskCoordinationEnabled ?? false,
    }
  })

  // Update a workspace setting
  // Valid keys: 'name', 'model', 'enabledSourceSlugs', 'permissionMode', 'cyclablePermissionModes', 'thinkingLevel', 'workingDirectory', 'localMcpEnabled', 'taskCoordinationEnabled'
  ipcMain.handle(IPC_CHANNELS.WORKSPACE_SETTINGS_UPDATE, async (_event, workspaceId: string, key: string, value: unknown) => {
    ...
    const validKeys = ['name', 'model', 'enabledSourceSlugs', 'permissionMode', 'cyclablePermissionModes', 'thinkingLevel', 'workingDirectory', 'localMcpEnabled', 'taskCoordinationEnabled']
