 ...
 * Deep Link Handler
 *
 * Parses vesper:// URLs and routes to appropriate actions.
 *
 * URL Formats (workspace is optional - uses active window if omitted):
 *
 * Compound format (hierarchical navigation):
 *   vesper://allChats[/chat/{sessionId}]            - Chat list (all chats)
 *   vesper://flagged[/chat/{sessionId}]             - Chat list (flagged filter)
 *   vesper://state/{stateId}[/chat/{sessionId}]     - Chat list (state filter)
 *   vesper://sources[/source/{sourceSlug}]          - Sources list
 *   vesper://settings[/{subpage}]                   - Settings (general, shortcuts, preferences)
 *
 * Action format:
 *   vesper://action/{actionName}[/{id}][?params]
 *   vesper://workspace/{workspaceId}/action/{actionName}[?params]
 *
 * Actions:
 ...
 *   vesper://allChats/chat/abc123                   (specific chat)
 *   vesper://settings/shortcuts                     (shortcuts page)
 *   vesper://sources/source/github                  (github source info)
 *   vesper://action/new-chat                        (uses active window)
 *   vesper://action/resume-sdk-session/{sdkId}      (resume Claude Code session)
 *   vesper://workspace/ws123/allChats/chat/abc123   (targets specific workspace)
 */
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    if (parsed.protocol !== 'vesper:') {
      return null
    }

    // For custom protocols, the hostname contains the first path segment
    // e.g., vesper://workspace/ws123 → hostname='workspace', pathname='/ws123'
    // e.g., vesper://allChats/chat/abc → hostname='allChats', pathname='/chat/abc'
    const host = parsed.hostname
    const pathParts = parsed.pathname.split('/').filter(Boolean)
    const windowMode = parseWindowMode(parsed)
    const rightSidebar = parseRightSidebar(parsed)

    // vesper://auth-callback?... (OAuth callbacks - return null to let existing handler process)
    ...
    // vesper://allChats/..., vesper://settings/..., etc. (compound routes)
    ...
    // vesper://workspace/{workspaceId}/... (with workspace targeting)
    if (host === 'workspace') {
    ...
    // vesper://action/... (no workspace - uses active window)
    if (host === 'action') {
