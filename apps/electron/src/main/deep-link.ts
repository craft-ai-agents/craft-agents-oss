 ...
 * Deep Link Handler
 *
 * Parses archstudio:// URLs and routes to appropriate actions.
 *
 * URL Formats (workspace is optional - uses active window if omitted):
 *
 * Compound format (hierarchical navigation):
 *   archstudio://allSessions[/session/{sessionId}]            - Session list (all sessions)
 *   archstudio://flagged[/session/{sessionId}]             - Session list (flagged filter)
 *   archstudio://state/{stateId}[/session/{sessionId}]     - Session list (state filter)
 *   archstudio://sources[/source/{sourceSlug}]          - Sources list
 *   archstudio://settings[/{subpage}]                   - Settings (general, shortcuts, preferences)
 *
 * Action format:
 *   archstudio://action/{actionName}[/{id}][?params]
 *   archstudio://workspace/{workspaceId}/action/{actionName}[?params]
 *
 * Actions:
 ...
 *   archstudio://allSessions/session/abc123                (specific session)
 *   archstudio://settings/shortcuts                     (shortcuts page)
 *   archstudio://sources/source/github                  (github source info)
 *   archstudio://action/new-chat                        (uses active window)
 *   archstudio://action/resume-sdk-session/{sdkId}      (resume Claude Code session)
 *   archstudio://workspace/ws123/allSessions/session/abc123   (targets specific workspace)
 */
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    if (parsed.protocol !== 'archstudio:') {
      return null
    }

    // For custom protocols, the hostname contains the first path segment
    // e.g., archstudio://workspace/ws123 → hostname='workspace', pathname='/ws123'
    // e.g., archstudio://allSessions/chat/abc → hostname='allSessions', pathname='/chat/abc'
    const host = parsed.hostname
    const pathParts = parsed.pathname.split('/').filter(Boolean)
    const windowMode = parseWindowMode(parsed)
    const rightSidebar = parseRightSidebar(parsed)

    // archstudio://auth-callback?... (OAuth callbacks - return null to let existing handler process)
    ...
    // archstudio://allSessions/..., archstudio://settings/..., etc. (compound routes)
    ...
    // archstudio://workspace/{workspaceId}/... (with workspace targeting)
    if (host === 'workspace') {
    ...
    // archstudio://action/... (no workspace - uses active window)
    if (host === 'action') {
