 ...
 * Includes only relevant items for the G4 OS app.
 *
 ...
 *
 * On Windows/Linux: Menu is hidden - all functionality is in the G4 OS logo menu.
...
export async function rebuildMenu(): Promise<void> {
  ...
  // Users access menu via the G4 OS logo dropdown in the app
  if (!isMac) {
  ...
  const template: Electron.MenuItemConstructorOptions[] = [
    ...
    ...(isMac ? [{
      label: 'G4 OS',
      submenu: [
        { role: 'about' as const, label: 'About G4 OS' },
        updateMenuItem,
        ...
        { type: 'separator' as const },
        { role: 'hide' as const, label: 'Hide G4 OS' },
        { role: 'hideOthers' as const },
        { role: 'unhide' as const },
        { type: 'separator' as const },
        { role: 'quit' as const, label: 'Quit G4 OS' }
      ]
    }] : []),
      ...
      submenu: [
        {
          ...
          click: () => shell.openExternal('https://g4educacao.com/docs')
        },
        {
