...
export default function PermissionsSettingsPage() {
  ...
  return (
    <div className="h-full flex flex-col">
      <PanelHeader title={t('settings.permissions.title', 'Permissions')} actions={<HeaderMenu route={routes.view.settings('permissions')} helpFeature="permissions" />} />
          ...
          <div className="px-5 py-7 max-w-3xl mx-auto">
            <div className="space-y-8">
              {isLoading ? (
                ...
                <>
                  {/* About Section */}
                  <SettingsSection title={t('settings.permissions.aboutPermissions', 'About Permissions')}>
                    <SettingsCard className="px-4 py-3.5">
                      <div className="text-sm text-muted-foreground leading-relaxed space-y-1.5">
                        <p>
                          {t('settings.permissions.permissionsControlAutonomy', 'Permissions control how much autonomy your agent has. In')} <span className="text-foreground/80 font-medium">{t('settings.permissions.explore', 'Explore')}</span> {t('settings.permissions.exploreModeDescription', 'mode, the agent can only read and research — perfect for understanding a problem before committing to changes. When you\'re ready, switch to')} <span className="text-foreground/80 font-medium">{t('settings.permissions.execute', 'Execute')}</span> {t('settings.permissions.executeModeDescription', 'mode to let the agent implement the plan autonomously.')}
                        </p>
                        <p>
                          {t('settings.permissions.goodWorkflow', 'A good workflow: start in Explore to let the agent investigate, review the proposed plan, then execute with confidence.')}
                        </p>
                        <p>
                          <button
                            type="button"
                          ...
                          >
                            {t('settings.permissions.learnMore', 'Learn more')}
                          </button>
                        </p>
                      </div>
                    </SettingsCard>
                  ...
                  <SettingsSection
                    title={t('settings.permissions.defaultPermissions', 'Default Permissions')}
                    description={t('settings.permissions.defaultPermissionsDescription', 'App-level patterns allowed in Explore mode. Commands not on this list are blocked.')}
                    action={
                      ...
                      defaultPermissionsPath ? (
                        <EditPopover
                          ...
                          secondaryAction={{
                            label: t('settings.permissions.editFile', 'Edit File'),
                    ...
                    <SettingsCard className="p-0">
                      {defaultPermissionsData.length > 0 ? (
                        <PermissionsDataTable
                          ...
                          maxHeight={350}
                          fullscreen
                          fullscreenTitle={t('settings.permissions.defaultPermissions', 'Default Permissions')}
                        />
                      ) : (
                        <div className="p-8 text-center text-muted-foreground">
                          <p className="text-sm">{t('settings.permissions.noDefaultPermissionsFound', 'No default permissions found.')}</p>
                          <p className="text-xs mt-1 text-foreground/40">
                            {t('settings.permissions.defaultPermissionsShouldBeAt', 'Default permissions should be at')} <code className="bg-foreground/5 px-1 rounded">~/.craft-agent/permissions/default.json</code>
                          </p>
                        </div>
                      )}
                    </SettingsCard>
                  ...
                  <SettingsSection
                    title={t('settings.permissions.workspaceCustomizations', 'Workspace Customizations')}
                    description={t('settings.permissions.workspaceCustomizationsDescription', 'Workspace-level patterns that extend the app defaults above.')}
                    action={
                      (() => {
                    ...
                    <SettingsCard className="p-0">
                      {customPermissionsData.length > 0 ? (
                        <PermissionsDataTable
                          ...
                          maxHeight={350}
                          fullscreen
                          fullscreenTitle={t('settings.permissions.workspaceCustomizations', 'Workspace Customizations')}
                        />
                      ) : (
                        <div className="p-8 text-center text-muted-foreground">
                          <p className="text-sm">{t('settings.permissions.noCustomPermissionsConfigured', 'No custom permissions configured.')}</p>
                          <p className="text-xs mt-1 text-foreground/40">
                            {t('settings.permissions.createPermissionsJson', 'Create a')} <code className="bg-foreground/5 px-1 rounded">permissions.json</code> {t('settings.permissions.fileInWorkspace', 'file in your workspace to add custom rules.')}
                          </p>
                        </div>
                      )}
                    </SettingsCard>
