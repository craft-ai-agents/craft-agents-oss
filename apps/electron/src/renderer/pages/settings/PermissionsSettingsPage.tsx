 ...
 */
function buildDefaultPermissionsData(
  ...
  config.allowedWritePaths?.forEach((item) => {
    ...
    rows.push({
      access: 'allowed',
      type: 'tool',
      pattern: t('permissions.table.writeTo', { path: pattern }),
      comment,
    })
  })
 ...
 */
function buildCustomPermissionsData(
  ...
  config.blockedTools?.forEach((item) => {
    ...
    const comment = typeof item === 'string'
      ? t('permissions.table.customBlockedTool')
      : (item.comment || t('permissions.table.customBlockedTool'))
  ...
  config.allowedBashPatterns?.forEach((item) => {
    const pattern = typeof item === 'string' ? item : item.pattern
    const comment = typeof item === 'string'
      ? t('permissions.table.customBashPattern')
      : (item.comment || t('permissions.table.customBashPattern'))
    rows.push({ access: 'allowed', type: 'bash', pattern, comment })
  })
  ...
  config.allowedMcpPatterns?.forEach((item) => {
    const pattern = typeof item === 'string' ? item : item.pattern
    const comment = typeof item === 'string'
      ? t('permissions.table.customMcpPattern')
      : (item.comment || t('permissions.table.customMcpPattern'))
    rows.push({ access: 'allowed', type: 'mcp', pattern, comment })
  })
  ...
  config.allowedApiEndpoints?.forEach((item) => {
    ...
    rows.push({
      ...
      type: 'api',
      pattern,
      comment: item.comment || t('permissions.table.customApiEndpoint'),
    })
  })
  ...
  config.allowedWritePaths?.forEach((item) => {
    const pattern = typeof item === 'string' ? item : item.pattern
    const comment = typeof item === 'string'
      ? t('permissions.table.allowedWritePath')
      : (item.comment || t('permissions.table.allowedWritePath'))
...
export default function PermissionsSettingsPage() {
  ...
  // Build default permissions data from ~/.craft-agent/permissions/default.json
  const defaultPermissionsData = useMemo(
    () => buildDefaultPermissionsData(defaultConfig, t),
    [defaultConfig, t, i18n.language]
  )
            ...
            <div className="space-y-8">
              {isLoading ? (
                ...
                <>
                  ...
                  <SettingsSection title={t('permissions.about.title')}>
                    <SettingsCard className="px-4 py-3.5">
                      <div className="text-sm text-muted-foreground leading-relaxed space-y-1.5">
                        <p>
                          <Trans
                            i18nKey="permissions.about.body"
                            t={t}
                            components={{
                              explore: <span className="text-foreground/80 font-medium" />,
                              execute: <span className="text-foreground/80 font-medium" />,
                            }}
                          />
                        </p>
                        <p>
                          {t('permissions.about.workflow')}
                        </p>
                        <p>
                          <button
                            type="button"
                  ...
                  <SettingsSection
                    title={t('permissions.default.title')}
                    description={t('permissions.default.description')}
                    action={
                      ...
                      defaultPermissionsPath ? (
                        <EditPopover
                          ...
                          secondaryAction={{
                            label: t('permissions.default.editFile'),
                            onClick: () => {
                    ...
                    <SettingsCard className="p-0">
                      {defaultPermissionsData.length > 0 ? (
                        <PermissionsDataTable
                          ...
                          maxHeight={350}
                          fullscreen
                          fullscreenTitle={t('permissions.default.title')}
                        />
                      ) : (
                        <div className="p-8 text-center text-muted-foreground">
                          <p className="text-sm">{t('permissions.default.empty.title')}</p>
                          <p className="text-xs mt-1 text-foreground/40">
                            <Trans
                              i18nKey="permissions.default.empty.hint"
                              t={t}
                              components={{
                                code: <code className="bg-foreground/5 px-1 rounded" />,
                              }}
                            />
                          </p>
                        </div>
                  ...
                  <SettingsSection
                    title={t('permissions.custom.title')}
                    description={t('permissions.custom.description')}
                    action={
                      (() => {
                        ...
                        return (
                          <EditPopover
                            ...
                            secondaryAction={activeWorkspace ? {
                              label: t('permissions.custom.editFile'),
                              onClick: () => {
                    ...
                    <SettingsCard className="p-0">
                      {customPermissionsData.length > 0 ? (
                        <PermissionsDataTable
                          ...
                          maxHeight={350}
                          fullscreen
                          fullscreenTitle={t('permissions.custom.title')}
                        />
                      ) : (
                        <div className="p-8 text-center text-muted-foreground">
                          <p className="text-sm">{t('permissions.custom.empty.title')}</p>
                          <p className="text-xs mt-1 text-foreground/40">
                            <Trans
                              i18nKey="permissions.custom.empty.hint"
                              t={t}
                              components={{
                                code: <code className="bg-foreground/5 px-1 rounded" />,
                              }}
