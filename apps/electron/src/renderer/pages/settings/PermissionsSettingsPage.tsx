 ...
 * Shows both default patterns (from ~/.g4os/permissions/default.json)
 * and custom workspace additions (from workspace permissions.json).
 *
 * Default patterns can be edited by the user in ~/.g4os/permissions/default.json.
...
import { type PermissionsConfigFile } from '@g4os/shared/agent/modes'
import {
...
/**
 * Build default permissions data from ~/.g4os/permissions/default.json.
          ...
          <div className="px-5 py-7 max-w-3xl mx-auto">
            <div className="space-y-8">
              {isLoading ? (
                ...
                <>
                  {/* About Section */}
                  <SettingsSection title={t('settings.permissions.section.about')}>
                    <SettingsCard className="px-4 py-3.5">
                      <div className="text-sm text-muted-foreground leading-relaxed space-y-1.5">
                        <p>
                          {t('settings.permissions.aboutP1').split(/<explore>|<\/explore>|<execute>|<\/execute>/).map((part, i) =>
                            i === 1 ? <span key={i} className="text-foreground/80 font-medium">{part}</span> :
                            i === 3 ? <span key={i} className="text-foreground/80 font-medium">{part}</span> :
                        ...
                        <p>
                          ...
                          >
                            {t('settings.permissions.learnMore')}
                          </button>
                        </p>
                      </div>
                    </SettingsCard>
                  ...
                  <SettingsSection
                    title={t('settings.permissions.section.default')}
                    description={t('settings.permissions.section.default.description')}
                    action={
                    ...
                    <SettingsCard className="p-0">
                      {defaultPermissionsData.length > 0 ? (
                        ...
                        <div className="p-8 text-center text-muted-foreground">
                          <p className="text-sm">{t('settings.permissions.noDefaults')}</p>
                          <p className="text-xs mt-1 text-foreground/40">
                            {t('settings.permissions.noDefaultsHint')}
                          </p>
                        </div>
                      )}
                    </SettingsCard>
                  ...
                  <SettingsSection
                    title={t('settings.permissions.section.custom')}
                    description={t('settings.permissions.section.custom.description')}
                    action={
                      (() => {
                  ...
                  >
                    <SettingsCard className="p-0">
                      {customPermissionsData.length > 0 ? (
                        ...
                        <div className="p-8 text-center text-muted-foreground">
                          <p className="text-sm">{t('settings.permissions.noCustom')}</p>
                          <p className="text-xs mt-1 text-foreground/40">
                            {t('settings.permissions.noCustomHint')}
                          </p>
                        </div>
                      )}
                    </SettingsCard>
