 ...
 *
 * Replaces Monaco Editor for code display with a lighter weight solution.
 * Uses the existing CodeBlock component from @craft-agent/ui for highlighting.
 *
 * Features:
 * - Line numbers
...
export function ShikiCodeViewer({
  ...
  }, [code, resolvedLang, resolvedMode, onReady])

  // Background color to match Monaco's themes
  const backgroundColor = resolvedMode === 'dark' ? '#1e1e1e' : '#ffffff'
  const lineNumberColor = resolvedMode === 'dark' ? 'rgba(255,255,255,0.3)' : 'rgba(0,0,0,0.3)'
  const borderColor = resolvedMode === 'dark' ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.06)'
