...
export function SessionItem({
  ...
  const isUnread = hasUnreadMeta(item)
  const isPlanReady = item.lastMessageRole === 'plan'
  const previewText = getSessionPreviewText(item)
  const signalClass = item.isProcessing
    ? 'bg-accent shadow-[0_0_8px_color-mix(in_srgb,var(--accent)_45%,transparent)] motion-safe:animate-pulse'
    : hasPendingPrompt
      ? 'bg-info'
      : isPlanReady
        ? 'bg-success'
  ...
  return (
    ...
    <EntityRow
      ...
      separatorClassName="pl-[46px] pr-4 opacity-60"
      isSelected={isSelected}
      isInMultiSelect={isInMultiSelect}
      // The Sessions ledger draws a state-aware signal spine instead of the
      // generic selection bar. Project color remains on the outermost edge.
      suppressSelectionBar
      onMouseDown={handleClick}
      buttonProps={{
        ...itemProps,
        className: cn(
          "min-h-[68px] py-2.5 pl-3 pr-3 rounded-[10px] focus-visible:ring-1 focus-visible:ring-inset focus-visible:ring-ring/60",
          isSelected
            ? "bg-foreground/[0.045] shadow-[inset_0_0_0_1px_var(--border)]"
      ...
      overlay={
        <div
          className={cn(
            "absolute left-0.5 top-3 bottom-3 w-[2px] rounded-full pointer-events-none transition-[background-color,box-shadow,opacity] duration-150",
            signalClass,
          )}
          aria-hidden
        />
      ...
      icon={
        <>
          ...
          <div className={cn(
            ...
            (item.isProcessing || isUnread || isPlanReady || hasPendingPrompt)
              ? "opacity-100 ml-0"
      ...
      titleClassName={cn(
        ...
        (isUnread || item.isProcessing || hasPendingPrompt) ? "font-semibold text-foreground" : "font-medium text-foreground/90",
        item.isAsyncOperationOngoing && "animate-shimmer-text",
      )}
      subtitle={previewText ? (
        <span className={cn(
          "block truncate text-[11.5px] leading-[1.45]",
          item.isProcessing ? "text-foreground/65" : "text-foreground/48",
        )}>
          {previewText}
        </span>
      ) : undefined}
      titleSuffix={
        (projectName || hasMessagingBinding) ? (
          <div className="flex items-center gap-1">
            {projectName && (
              <span
                className="text-[10px] font-medium uppercase tracking-[0.08em] text-foreground/40 whitespace-nowrap truncate max-w-[120px] opacity-70 group-hover:opacity-100 transition-opacity duration-150"
      ...
      titleTrailing={hasMatch ? (
        ...
        <span className="text-[10px] font-medium tabular-nums text-foreground/38 whitespace-nowrap">
