...
export function SidebarMenu({
  ...
  if ((type === 'allChats' || type === 'status' || type === 'flagged') && onConfigureStatuses) {
    return (
      <MenuItem onClick={onConfigureStatuses}>
        ...
        <span className="flex-1">{t('menu.configureStatuses')}</span>
      </MenuItem>
    )
  }
      ...
      <>
        {onAddLabel && (
          <MenuItem onClick={() => onAddLabel(labelId)}>
            ...
            <span className="flex-1">{t('menu.addLabel')}</span>
          </MenuItem>
        )}
        {onConfigureLabels && (
          <MenuItem onClick={() => onConfigureLabels(labelId)}>
            <Settings2 className="h-3.5 w-3.5" />
            <span className="flex-1">{t('menu.editLabels')}</span>
          </MenuItem>
        )}
        {labelId && onDeleteLabel && (
          <>
            ...
            <MenuItem onClick={() => onDeleteLabel(labelId)}>
              ...
              <span className="flex-1">{t('menu.deleteLabel')}</span>
            </MenuItem>
          </>
        )}
      </>
      ...
      <>
        {onConfigureViews && (
          <MenuItem onClick={onConfigureViews}>
            <Settings2 className="h-3.5 w-3.5" />
            <span className="flex-1">{t('menu.editViews')}</span>
          </MenuItem>
        )}
        {viewId && onDeleteView && (
          <>
            ...
            <MenuItem onClick={() => onDeleteView(viewId)}>
              <Trash2 className="h-3.5 w-3.5" />
              <span className="flex-1">{t('menu.deleteView')}</span>
            </MenuItem>
          </>
        )}
      </>
  ...
  if (type === 'sources') {
    ...
    const learnMoreLabel = sourceType === 'api'
      ? t('menu.learnMoreApis')
      : sourceType === 'mcp'
        ? t('menu.learnMoreMcp')
        : sourceType === 'local'
          ? t('menu.learnMoreLocalFolders')
          : t('menu.learnMoreSources')
      ...
      <>
        {onAddSource && (
          <MenuItem onClick={onAddSource}>
            <Plus className="h-3.5 w-3.5" />
            <span className="flex-1">{t('menu.addSource')}</span>
          </MenuItem>
        )}
        <Separator />
  ...
  if (type === 'skills' && onAddSkill) {
    return (
      <MenuItem onClick={onAddSkill}>
        <Plus className="h-3.5 w-3.5" />
        <span className="flex-1">{t('menu.addSkill')}</span>
      </MenuItem>
    )
  }
