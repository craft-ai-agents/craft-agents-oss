...
export function DesktopAccountLogin({ onErpLogin, onLocalLogin }: Props) {
  ...
  const [username, setUsername] = useState(isDevelopment ? 'local-admin' : '')
  const [password, setPassword] = useState(isDevelopment ? 'JonworkLocal@2026' : '')
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-foreground-2 p-6">
      <section className="w-full max-w-sm space-y-4 rounded-2xl bg-background p-7 shadow-modal-small">
        <div>
          <h1 className="text-xl font-semibold">{isDevelopment ? '登录本地 Jonwork' : '连接 ERPNext'}</h1>
          <p className="mt-1 text-sm text-muted-foreground">{isDevelopment ? '本地开发账号已自动填充，可直接登录验证桌面端。' : 'Craft 不维护独立账号。请先登录 ERPNext，再同步企业、权限、项目和积分到 Craft。'}</p>
        ...
        {isDevelopment && <div className="space-y-3">
        ...
        <Button type="button" className="w-full" disabled={loading || !serverUrl || (isDevelopment && (!username || !password))} onClick={async () => {
          setError(''); setLoading(true)
          try {
            const normalized = normalizeAccountServerUrl(serverUrl)
            if (isDevelopment) await onLocalLogin(normalized, username, password)
            else await onErpLogin(normalized)
          }
          catch (cause) { setError(formatLoginError(cause)) }
          finally { setLoading(false) }
        }}>{loading ? (isDevelopment ? '正在登录…' : '正在从 ERPNext 同步，最长等待 5 分钟…') : (isDevelopment ? '登录本地 Jonwork' : '登录 ERPNext 并同步到 Craft')}</Button>
