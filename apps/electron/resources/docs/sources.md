...
This guide explains how to configure sources (MCP servers, APIs, local filesystems) in ARCHstudio.
...
- `~/.archstudio/workspaces/{workspaceId}/sources/{sourceSlug}/`
...
`~/.archstudio/provider-domains.json`
...
1. Create the source folder:
   ```bash
   mkdir -p ~/.archstudio/workspaces/{ws}/sources/my-source
   ```
