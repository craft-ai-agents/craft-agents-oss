...
This guide explains how to configure automations in ARCHstudio to automate workflows based on events.
...
Automations allow you to trigger actions automatically when specific events occur in ARCHstudio. You can:
...
```
~/.archstudio/workspaces/{workspaceId}/automations.json
```
...
### App Events (triggered by ARCHstudio)
...
Send a prompt to ARCHstudio (creates a new session for scheduled prompts).
...
- Environment variables are expanded (e.g., `$ARCHSTUDIO_LABEL`)
...
{
  "type": "webhook",
  "url": "https://hooks.slack.com/services/${ARCHSTUDIO_WH_SLACK_PATH}",
  "method": "POST",
  "body": {
    "text": "Session ${ARCHSTUDIO_SESSION_NAME} status changed to ${ARCHSTUDIO_NEW_STATE}"
  }
}
```
...
```json
{
  ...
  "auth": {
    "type": "bearer",
    "token": "${ARCHSTUDIO_WH_API_TOKEN}"
  },
  "body": { "event": "$ARCHSTUDIO_EVENT" }
}
```
...
```json
{
  ...
  "auth": {
    "type": "basic",
    "username": "${ARCHSTUDIO_WH_USER}",
    "password": "${ARCHSTUDIO_WH_PASS}"
  }
}
```
...
**Security:** Webhook actions only have access to `ARCHSTUDIO_*` system variables and `ARCHSTUDIO_WH_*` user-defined secrets. They do **not** have access to your full system environment (e.g., `$HOME`, `$PATH`, or other process variables).

## Environment Variables

Both prompt and webhook actions support variable expansion using `$VAR` or `${VAR}` syntax.

### System Variables (ARCHSTUDIO_*)

These are automatically set by the automation system based on the triggering event:

| Variable | Description | Available For |
|----------|-------------|---------------|
| `$ARCHSTUDIO_EVENT` | Event name (e.g., `LabelAdd`) | All events |
| `$ARCHSTUDIO_EVENT_DATA` | Full event payload as JSON | All events |
| `$ARCHSTUDIO_SESSION_ID` | Session ID | Events with session context |
| `$ARCHSTUDIO_SESSION_NAME` | Session name | Events with session context |
| `$ARCHSTUDIO_WORKSPACE_ID` | Workspace ID | All events |

**Per-event variables:**

| Event | Variable | Description |
|-------|----------|-------------|
| `LabelAdd` / `LabelRemove` | `$ARCHSTUDIO_LABEL` | The label that was added/removed |
| `PermissionModeChange` | `$ARCHSTUDIO_OLD_MODE`, `$ARCHSTUDIO_NEW_MODE` | Previous and new permission mode |
| `FlagChange` | `$ARCHSTUDIO_IS_FLAGGED` | `true` or `false` |
| `SessionStatusChange` | `$ARCHSTUDIO_OLD_STATE`, `$ARCHSTUDIO_NEW_STATE` | Previous and new status |
| `SchedulerTick` | `$ARCHSTUDIO_LOCAL_TIME`, `$ARCHSTUDIO_LOCAL_DATE` | Current time (`14:30`) and date (`2026-03-09`) |

### User-Defined Webhook Secrets (ARCHSTUDIO_WH_*)

For webhook actions, you can define your own secrets by setting environment variables with the `ARCHSTUDIO_WH_` prefix in your shell profile (e.g., `~/.zshrc`, `~/.bashrc`):

```bash
# In your shell profile
export ARCHSTUDIO_WH_SLACK_URL="https://hooks.slack.com/services/T.../B.../xxx"
export ARCHSTUDIO_WH_DISCORD_URL="https://discord.com/api/webhooks/123/abc"
export ARCHSTUDIO_WH_API_TOKEN="your-secret-token"
```
...
```json
{
  ...
  "method": "POST",
  "body": { "text": "Hello from ARCHstudio!" }
}
```
...
```json
{
  "type": "webhook",
  "url": "https://api.example.com/events",
  "headers": { "Authorization": "Bearer ${ARCHSTUDIO_WH_API_TOKEN}" },
  "body": { "event": "${ARCHSTUDIO_EVENT}", "session": "${ARCHSTUDIO_SESSION_NAME}" }
}
```

This keeps secrets out of `automations.json` (which may be shared or committed to version control).

> **Note:** Only variables prefixed with `ARCHSTUDIO_WH_` are injected into webhook actions. Other environment variables (like `$HOME` or `$DATABASE_URL`) are not accessible to webhooks.
...
4. **Pair the supergroup.** In ARCHstudio: Settings → Messaging → Telegram → **Pair Supergroup**. Copy the 6-digit code, then in any topic of the supergroup type `/pair <code>`. The bot confirms and the Settings row updates with the group's title.

Verify by checking the supergroup row in Settings shows the group title. If automation runs fail later, `~/.archstudio/logs/messaging-gateway.log` will show `automation_topic_bind_failed` with the underlying Telegram error.
        ...
        "actions": [
          {
            ...
            "url": "${ARCHSTUDIO_WH_SLACK_URL}",
            "method": "POST",
            "body": { "text": ":warning: Permission escalated from safe to allow-all in *${ARCHSTUDIO_SESSION_NAME}*" }
          }
        ]
      }
    ]
...
```json
{
  ...
  "automations": {
    "LabelAdd": [
      {
        "actions": [
          { "type": "prompt", "prompt": "The label $ARCHSTUDIO_LABEL was added. Log this change with a timestamp." }
        ]
      }
    ],
    "LabelRemove": [
      {
        "actions": [
          { "type": "prompt", "prompt": "The label $ARCHSTUDIO_LABEL was removed. Log this change with a timestamp." }
        ]
      }
    ]
  }
...
Sends a Slack message when a session is marked as done. Requires `ARCHSTUDIO_WH_SLACK_URL` in your shell profile.
        ...
        "actions": [
          {
            ...
            "method": "POST",
            "body": {
              "text": ":white_check_mark: Session *${ARCHSTUDIO_SESSION_NAME}* marked as done"
            }
          }
        ]
      }
        ...
        "actions": [
          {
            ...
            "url": "${ARCHSTUDIO_WH_SLACK_URL}",
            "method": "POST",
            "body": { "text": ":rotating_light: Urgent label added to *${ARCHSTUDIO_SESSION_NAME}*" }
          },
          {
            "type": "prompt",
        ...
        "actions": [
          {
            ...
            "body": {
              ...
              "client_id": "${ARCHSTUDIO_WH_CLIENT_ID}",
              "client_secret": "${ARCHSTUDIO_WH_CLIENT_SECRET}"
            }
          }
        ]
      }
        ...
        "actions": [
          {
            ...
            "method": "POST",
            "headers": {
              "Authorization": "Bearer ${ARCHSTUDIO_WH_API_TOKEN}",
              "X-Source": "craft-agent"
            },
            "body": {
              "event": "${ARCHSTUDIO_EVENT}",
              "session_id": "${ARCHSTUDIO_SESSION_ID}",
              "old_status": "${ARCHSTUDIO_OLD_STATE}",
              "new_status": "${ARCHSTUDIO_NEW_STATE}"
            }
          }
        ]
      }
...
Ask ARCHstudio to validate your automations configuration:
...
2. **Check env vars** — Ensure `ARCHSTUDIO_WH_*` variables are set in your shell profile and ARCHstudio was restarted after adding them. URLs using `$VAR` templates are validated after variable expansion — if the variable is empty or unset, the URL will be invalid.
...
5. **Keep secrets out of config** - Use `ARCHSTUDIO_WH_*` env vars for webhook URLs and tokens instead of hardcoding them in automations.json
