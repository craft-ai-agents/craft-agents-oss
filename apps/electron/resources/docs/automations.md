...
This guide explains how to configure automations in Runner to automate workflows based on events.
...
Automations allow you to trigger actions automatically when specific events occur in Runner. You can:
...
### App Events (triggered by Runner)
...
Send a prompt to Runner (creates a new session for scheduled prompts).
...
Ask Runner to validate your automations configuration:
...
2. **Check env vars** — Ensure `CRAFT_WH_*` variables are set in your shell profile and Runner was restarted after adding them. URLs using `$VAR` templates are validated after variable expansion — if the variable is empty or unset, the URL will be invalid.
