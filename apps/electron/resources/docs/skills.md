...
This guide explains how to create and configure skills in ARCHstudio.
...
ARCHstudio uses **the identical SKILL.md format** as the Claude Code SDK. This means:

1. **Format compatibility**: Any skill written for Claude Code works in ARCHstudio
...
1. **Workspace skill checked first** - If `~/.archstudio/workspaces/{id}/skills/commit/SKILL.md` exists, it's used
...
```
~/.archstudio/workspaces/{workspaceId}/skills/{slug}/
...
```bash
mkdir -p ~/.archstudio/workspaces/{ws}/skills/my-skill
```
...
1. Create `~/.archstudio/workspaces/{ws}/skills/commit/SKILL.md`
