...
This guide explains how to create and configure skills in G4 OS.
...
G4 OS uses **the identical SKILL.md format** as the Claude Code SDK. This means:

1. **Format compatibility**: Any skill written for Claude Code works in G4 OS
...
1. **Workspace skill checked first** - If `~/.g4os/workspaces/{id}/skills/commit/SKILL.md` exists, it's used
...
```
~/.g4os/workspaces/{workspaceId}/skills/{slug}/
...
```bash
mkdir -p ~/.g4os/workspaces/{ws}/skills/my-skill
```
...
1. Create `~/.g4os/workspaces/{ws}/skills/commit/SKILL.md`
