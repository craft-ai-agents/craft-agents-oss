...
This guide explains how to customize the visual theme of G4 OS.

## Overview

G4 OS uses a 6-color theme system with support for both app-level defaults and per-workspace overrides.
...
3. **Preset themes**: `~/.g4os/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `~/.g4os/theme.json` - Override specific colors (app-level)
...
```
~/.g4os/workspaces/{id}/config.json
```
...
Create `~/.g4os/theme.json` to override specific colors:
...
Preset themes are complete theme packages stored at `~/.g4os/themes/`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to `~/.g4os/themes/{name}.json`
...
1. Create `~/.g4os/theme.json` for overrides or `~/.g4os/themes/{name}.json` for a preset
...
- Check file is in correct location (`~/.g4os/theme.json` for overrides, `~/.g4os/themes/` for presets)
