...
This guide explains how to customize the visual theme of ARCHstudio.

## Overview

ARCHstudio uses a 6-color theme system with support for both app-level defaults and per-workspace overrides.
...
3. **Preset themes**: `~/.archstudio/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `~/.archstudio/theme.json` - Override specific colors (app-level)
...
```
~/.archstudio/workspaces/{id}/config.json
```
...
Create `~/.archstudio/theme.json` to override specific colors:
...
Preset themes are complete theme packages stored at `~/.archstudio/themes/`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to `~/.archstudio/themes/{name}.json`
...
1. Create `~/.archstudio/theme.json` for overrides or `~/.archstudio/themes/{name}.json` for a preset
...
- Check file is in correct location (`~/.archstudio/theme.json` for overrides, `~/.archstudio/themes/` for presets)
