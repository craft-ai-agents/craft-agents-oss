...
### Why it's cool
This is the first step toward full programmatic control of ARCHstudio. Script your agent workflows, integrate into CI/CD pipelines, or run sessions headlessly on remote machines. Partially addresses [#343](https://github.com/lukilabs/craft-agents-oss/issues/343).

## Headless Server

Run ARCHstudio as a standalone server without the Electron desktop app — deploy on any machine, connect from any client.
...
### Why it's cool
Deploy ARCHstudio on a remote server and connect from anywhere. Run it on your home lab, a cloud VM, or a CI runner. The thin-client mode means the browser UI stays lightweight while all the heavy lifting happens server-side. Partially addresses [#291](https://github.com/lukilabs/craft-agents-oss/issues/291).
...
- **Server-core extraction** — 14 core handlers, session management, services, and domain utilities extracted into `@archstudio/server-core` for reuse across Electron and headless
- **Protocol formalization** — channels, DTOs, and event maps extracted to `@archstudio/shared/protocol` with wire-format stability tests
