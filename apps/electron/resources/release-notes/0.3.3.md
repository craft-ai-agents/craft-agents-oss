...
When your OAuth token expires, MDP automatically refreshes it in the background using the refresh token. No manual re-authentication required — your MCP connections stay alive.
...
Configure sources that require multiple authentication headers (e.g., API key + tenant ID). See the updated documentation at `~/.mdp-agent/docs/sources.md`.
