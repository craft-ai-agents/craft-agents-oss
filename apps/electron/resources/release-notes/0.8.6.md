...
- **Image input for custom endpoints** — Custom endpoint models (e.g. **Gemma 4** via Ollama) can now receive image attachments. Previously, images were silently discarded because custom models hardcoded text-only input. **Important:** custom endpoints remain text-only by default — you must set `supportsImages: true` in the connection config to enable image input. See the [custom endpoint documentation](https://github.com/findmikeymike/RunnerOS/reference/config/custom-endpoint#image-input-for-custom-endpoints) for per-model and whole-endpoint examples. (Fixes #525)
