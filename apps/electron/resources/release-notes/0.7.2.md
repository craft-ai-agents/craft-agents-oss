...
- **Island system** — extracted `Island` and `IslandFollowUpContentView` into the shared `@archstudio/ui` package as reusable primitives for annotation follow-ups and inline content expansion (7c4ba6af, 2432b421, 4397b284)
...
- **Local dev build mode** — new `ARCHSTUDIO_DEV_RUNTIME` build flag resolves SDK and interceptor from the monorepo during development, with platform-aware bun fallback; adds `electron:dist:dev:{mac,win,linux}` scripts (506f2b02)
