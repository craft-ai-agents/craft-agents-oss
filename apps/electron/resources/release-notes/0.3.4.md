...
Place your skills in `~/.agents/skills/` and they'll work with ARCHstudio, Codex, Gemini, and other tools that support this convention. Your existing workspace skills continue to work as before.
...
When you set `colorTheme` in `~/.archstudio/config.json`, it now properly takes priority over any cached theme in localStorage. This makes it possible for agents to modify the theme via documented config paths.
...
| Issue | Title | Status |
|-------|-------|--------|
| [craft-agents-oss#131](https://github.com/lukilabs/craft-agents-oss/issues/131) | ARCHstudio on Hyprland creates multiple non-closable windows | Fixed |
