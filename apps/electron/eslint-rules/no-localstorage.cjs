 ...
 *
 * Warns against using localStorage in MDP Agent codebase.
 * All persistent user settings should be stored in file-based configs
 * (preferences.json, workspace configs) for consistency with MDP Agent
...
module.exports = {
  meta: {
    ...
    messages: {
      noLocalStorage:
        "Avoid localStorage in MDP Agent. Store settings in ~/.mdp-agent/preferences.json using window.electronAPI.readPreferences/writePreferences. See packages/shared/src/config/preferences.ts for the preferences API.",
    },
    schema: [],
  },
