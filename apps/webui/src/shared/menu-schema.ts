 ...
 * Shared Menu Schema
 *
 * Defines menu structure consumed by the WebUI renderer.
 *
...
export interface MenuItemAction {
  ...
  actionId?: string
  shortcut: string              // Shortcut descriptor: 'CmdOrCtrl+B'
...
export interface MenuItemRole {
  type: 'role'
  role: string                  // Browser/native menu role: 'undo', 'copy', etc.
  labelKey: string              // i18n key — resolve with t() at render time
 ...
 *
 * Renderers turn this into `window.webAgentAPI.openUrl(url)`.
 */
...
/**
 * Debug submenu — only rendered when `window.webAgentAPI.isDebugMode()` resolves true.
 *
 * The web app has no native menu; debug actions call WebAgentAPI directly.
 */
