 ...
 * IPC mocks. Phase 3 wiring will read the same state via webAgentAPI.
 */
...
export const mockWebAgentAPI = {
  isDebugMode: async () => true,

  // Called at module-load time by SessionFilesSection.tsx (and others) to
  // branch between web and other host rendering. Must be synchronous.
  getRuntimeEnvironment: (): 'web' => 'web',
  ...
  // ------------------------------------------------------------------
  // Messaging access control (mirrors the production WebAgentAPI surface)
  // ------------------------------------------------------------------
...
/**
 * Inject mock webAgentAPI into window if not already present.
 * Call this in playground component wrappers before rendering components
 * that depend on webAgentAPI.
 *
 ...
 * `window.webAgentAPI.*` read at module-load time (e.g.
 ...
 * `window.webAgentAPI` at import time.
 */
export function ensureMockWebAgentAPI() {
  if (!window.webAgentAPI) {
    ;(window as any).webAgentAPI = mockWebAgentAPI
    console.log('[Playground] Injected mock webAgentAPI')
  }
...
// Install on import so any later module that reads `window.webAgentAPI` at
// top level finds the mock in place. Safe: only runs once (idempotent).
ensureMockWebAgentAPI()
