...
function AppShellContent({
  ...
  React.useEffect(() => {
    ...
    window.webAgentAPI.getWorkspaceSettings(activeWorkspaceId).then((settings) => {
      if (settings) {
  ...
  React.useEffect(() => {
    if (!activeWorkspaceId) return
    window.webAgentAPI.getSources(activeWorkspaceId).then((loaded) => {
      setSources(loaded || [])
    }).catch(err => {
  ...
  React.useEffect(() => {
    const cleanup = window.webAgentAPI.onSourcesChanged((workspaceId, updatedSources) => {
  ...
  React.useEffect(() => {
    const cleanup = window.webAgentAPI.onSkillsChanged((workspaceId, updatedSkills) => {
      if (workspaceId !== activeWorkspaceId) return
  ...
  const handleSessionSourcesChange = React.useCallback(async (sessionId: string, sourceSlugs: string[]) => {
    try {
      await window.webAgentAPI.sessionCommand(sessionId, { type: 'setSources', sourceSlugs })
  ...
  const handleSessionLabelsChange = React.useCallback(async (sessionId: string, labels: string[]) => {
    try {
      await window.webAgentAPI.sessionCommand(sessionId, { type: 'setLabels', labels })
  ...
  // New window
  useAction('app.newWindow', () => window.webAgentAPI.menuNewWindow())

  // Quit (note: also handled by native menu on macOS)
  useAction('app.quit', () => window.webAgentAPI.menuQuit())
  ...
  useAction('chat.stopProcessing', () => {
    if (effectiveSessionId) {
      ...
      if (meta?.isProcessing) {
        ...
        if (shouldInterrupt) {
          window.webAgentAPI.cancelProcessing(effectiveSessionId, false).catch(err => {
  ...
  React.useEffect(() => {
    if (!activeWorkspaceId) return
    window.webAgentAPI.getSkills(activeWorkspaceId, activeSessionWorkingDirectory).then((loaded) => {
      setSkills(loaded || [])
    }).catch(err => {
  ...
  const refreshWorkspaceUnreadMap = useCallback(async () => {
    try {
      const summary = await window.webAgentAPI.getUnreadSummary()
  ...
  useEffect(() => {
    const cleanup = window.webAgentAPI.onUnreadSummaryChanged((summary) => {
      const next: Record<string, boolean> = {}
      for (const workspace of workspaces) {
        next[workspace.id] = !!summary.hasUnreadByWorkspace[workspace.id]
      }
  ...
  React.useEffect(() => {
    const cleanup = window.webAgentAPI.onMenuToggleFocusMode?.(() => {
  ...
  React.useEffect(() => {
    const cleanup = window.webAgentAPI.onMenuToggleSidebar?.(() => {
      handleToggleSidebar()
    })
    return cleanup
  }, [handleToggleSidebar])
  ...
  const handleStatusReorder = useCallback((orderedIds: string[]) => {
    ...
    window.webAgentAPI.reorderStatuses(activeWorkspaceId, orderedIds)
  }, [activeWorkspaceId])
  ...
  const handleDeleteView = useCallback(async (viewId: string) => {
    ...
    try {
      ...
      await window.webAgentAPI.saveViews(activeWorkspace.id, updated)
    } catch (err) {
  ...
  const handleDeleteLabel = useCallback(async (labelId: string) => {
    if (!activeWorkspace?.id) return
    try {
      await window.webAgentAPI.deleteLabel(activeWorkspace.id, labelId)
    } catch (err) {
  ...
  const handleDeleteSource = useCallback(async (sourceSlug: string) => {
    if (!activeWorkspace) return
    try {
      await window.webAgentAPI.deleteSource(activeWorkspace.id, sourceSlug)
  ...
  const handleDeleteSkill = useCallback(async (skillSlug: string) => {
    if (!activeWorkspace) return
    try {
      await window.webAgentAPI.deleteSkill(activeWorkspace.id, skillSlug)
  ...
  return (
    <AppShellProvider value={appShellContextValue}>
        ...
        <TopBar
          ...
          onNewWindow={() => window.webAgentAPI.menuNewWindow()}
                    ...
                    // All Sessions: expandable with status children (sortable) + Flagged & Archived as trailing items
                    {
                      ...
                      contextMenu: {
                        ...
                        onMarkAllRead: () => {
                          ...
                          window.webAgentAPI.markAllSessionsRead(activeWorkspaceId)
                        },
                      },
                ...
                <SessionList
                  ...
                  onOpenInNewWindow={(selectedMeta) => {
                    if (activeWorkspaceId) {
                      window.webAgentAPI.openSessionInNewWindow(activeWorkspaceId, selectedMeta.id)
                    }
                  }}
