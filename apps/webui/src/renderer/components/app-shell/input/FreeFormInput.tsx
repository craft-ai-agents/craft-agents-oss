...
export function FreeFormInput({
    ...
    const loadInputSettings = async () => {
      ...
      try {
        const [autoCapEnabled, sendKey, spellCheckEnabled] = await Promise.all([
          window.webAgentAPI.getAutoCapitalisation(),
          window.webAgentAPI.getSendMessageKey(),
          window.webAgentAPI.getSpellCheck(),
        ])
  ...
  React.useEffect(() => {
    const handleApprovePlanWithCompact = async (e: CustomEvent<PlanApprovalEventDetail>) => {
      ...
      if (sessionId) {
        await window.webAgentAPI.sessionCommand(sessionId, {
  ...
  React.useEffect(() => {
    ...
    const executePendingPlan = async () => {
      ...
      try {
        const pending = await window.webAgentAPI.getPendingPlanExecution(sessionId)
  ...
  React.useEffect(() => {
    ...
    window.webAgentAPI?.getHomeDir?.().then((dir: string) => {
  ...
  // Check if running in WebUI environment (has webAgentAPI)
  const hasWebAgentAPI = typeof window !== 'undefined' && !!window.webAgentAPI
  ...
  const readFileAsAttachment = async (file: File, overrideName?: string): Promise<FileAttachment | null> => {
    ...
    const realPath = hasWebAgentAPI ? window.webAgentAPI.getFilePath?.(file) ?? null : null
    ...
    return new Promise((resolve) => {
      ...
      reader.onload = async () => {
        ...
        if (hasWebAgentAPI) {
          try {
            const thumb = await window.webAgentAPI.generateThumbnail(base64, mimeType)
    ...
    <form onSubmit={handleSubmit}>
      ...
      >
        ...
        {/* Controlled EditPopover for "Add New Label" — opens when user selects
            ...
            fall back to the same-window custom-protocol path; that worked inside
            WebUI but could leave the current web session via `craftagents://`.
