...
function isStackedActivityTool(activity: ActivityItem): boolean {
  ...
  return toolName === 'bash' || toolName.startsWith('mcp__')
}
...
export const ChatDisplay = React.forwardRef<ChatDisplayHandle, ChatDisplayProps>(function ChatDisplay({
  ...
  useEffect(() => {
    ...
    const loadSendMessageKey = async () => {
      ...
      try {
        const key = await window.webAgentAPI.getSendMessageKey()
        if (!isMounted) return
  ...
  useEffect(() => {
    window.webAgentAPI.readPreferences().then(({ content }) => {
      try {
  ...
  const handleDiffViewerSettingsChange = useCallback((settings: DiffViewerSettings) => {
    ...
    window.webAgentAPI.readPreferences().then(({ content }) => {
      try {
        ...
        window.webAgentAPI.writePreferences(JSON.stringify(prefs, null, 2))
      } catch {
        // If preferences malformed, create fresh with just diffViewer
        window.webAgentAPI.writePreferences(JSON.stringify({ diffViewer: settings, updatedAt: Date.now() }, null, 2))
      }
    })
  }, [])
  ...
  const handleSubmit = (message: string, attachments?: FileAttachment[], skillSlugs?: string[]) => {
    ...
    if (session && pendingFollowUpAnnotations.length > 0) {
      ...
      void Promise.all(pendingFollowUpAnnotations.map((followUp) => {
        ...
        return window.webAgentAPI.sessionCommand(session.id, {
  ...
  const handleStop = (silent = false) => {
    ...
    window.webAgentAPI.cancelProcessing(session.id, silent).catch(error => {
                    ...
                    return (
                      ...
                      <TurnCard
                        ...
                        onAddAnnotation={async (messageId, annotation) => {
                          if (!session) return
                          try {
                            await window.webAgentAPI.sessionCommand(session.id, {
                              type: 'addAnnotation',
                              messageId,
                              annotation,
                            })
