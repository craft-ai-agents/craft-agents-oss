...
export default function AppearanceSettingsPage() {
  ...
  useEffect(() => {
    window.webAgentAPI?.getRichToolDescriptions?.().then(setRichToolDescriptions)
  }, [])
  const handleRichToolDescriptionsChange = useCallback(async (checked: boolean) => {
    setRichToolDescriptions(checked)
    await window.webAgentAPI?.setRichToolDescriptions?.(checked)
  }, [])
  ...
  useEffect(() => {
    const loadThemes = async () => {
      ...
      try {
        const themes = await window.webAgentAPI.loadPresetThemes()
  ...
  useEffect(() => {
    const loadWorkspaceThemes = async () => {
      if (!window.webAgentAPI?.getAllWorkspaceThemes) return
      try {
        const themes = await window.webAgentAPI.getAllWorkspaceThemes()
    ...
    const load = async () => {
      ...
      try {
        const [mappings, homeDir] = await Promise.all([
          window.webAgentAPI.getToolIconMappings(),
          window.webAgentAPI.getHomeDir(),
        ])
  ...
  const handleWorkspaceThemeChange = useCallback(
    async (workspaceId: string, value: string) => {
      ...
      if (workspaceId === activeWorkspaceId) {
        ...
        await window.webAgentAPI?.setWorkspaceColorTheme?.(workspaceId, themeId)
      }
                  ...
                  <SettingsRow label={t("settings.appearance.language")}>
                    <SettingsMenuSelect
                      ...
                      onValueChange={(value) => {
                        ...
                        window.webAgentAPI?.changeLanguage?.(value)
                      }}
