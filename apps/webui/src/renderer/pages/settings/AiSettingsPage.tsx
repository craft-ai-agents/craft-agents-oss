...
function ConnectionRow({ connection, isLastConnection, onRenameClick, onDelete, onSetDefault, onValidate, onReauthenticate, onEdit, onSetMidStreamBehavior, validationState, validationError, isDuplicateAccount }: ConnectionRowProps) {
  ...
  useEffect(() => {
    ...
    if (provider === 'pi' && connection.piAuthProvider && !connection.baseUrl) {
      window.webAgentAPI.getPiProviderBaseUrl(connection.piAuthProvider).then(url => setPiBaseUrl(url))
    }
...
function WorkspaceOverrideCard({ workspace, llmConnections, onSettingsChange }: WorkspaceOverrideCardProps) {
  ...
  useEffect(() => {
    const loadSettings = async () => {
      if (!window.webAgentAPI) return
      setIsLoading(true)
      try {
        const ws = await window.webAgentAPI.getWorkspaceSettings(workspace.id)
        setSettings(ws)
      } catch (error) {
  ...
  const updateSetting = useCallback(async <K extends keyof WorkspaceSettings>(key: K, value: WorkspaceSettings[K]) => {
    ...
    try {
      await window.webAgentAPI.updateWorkspaceSetting(workspace.id, key, value)
      onSettingsChange()
    } catch (error) {
...
export default function AiSettingsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const defaultThinkingLevel = await window.webAgentAPI.getDefaultThinkingLevel()
        setDefaultThinking(defaultThinkingLevel)

        const extendedCache = await window.webAgentAPI.getExtendedPromptCache()
        setExtendedPromptCache(extendedCache)

        const enable1M = await window.webAgentAPI.getEnable1MContext()
        setEnable1MContext(enable1M)

        const rtkOn = await window.webAgentAPI.getRtkEnabled()
        setRtkEnabled(rtkOn)

        const status = await window.webAgentAPI.getRtkStatus()
        setRtkStatus(status)

        // Check credential health for potential issues (corruption, machine migration)
        const health = await window.webAgentAPI.getCredentialHealth()
        if (!health.healthy) {
  ...
  const handleRenameSubmit = useCallback(async () => {
    if (!renamingConnection || !window.webAgentAPI) return
    ...
    try {
      ...
      const connection = await window.webAgentAPI.getLlmConnection(renamingConnection.slug)
      if (connection) {
        const result = await window.webAgentAPI.saveLlmConnection({ ...connection, name: trimmedName })
        if (result.success) {
          refreshLlmConnections?.()
        } else {
  ...
  const handleEditConnection = useCallback(async (connection: LlmConnectionWithStatus) => {
    ...
    try {
      apiKey = (await window.webAgentAPI.getLlmConnectionApiKey(connection.slug)) ?? undefined
    } catch {
  ...
  const handleDeleteConnection = useCallback(async (slug: string) => {
    if (!window.webAgentAPI) return
    try {
      const result = await window.webAgentAPI.deleteLlmConnection(slug)
      if (result.success) {
        refreshLlmConnections?.()
      } else {
  ...
  const handleValidateConnection = useCallback(async (slug: string) => {
    ...
    try {
      const result = await window.webAgentAPI.testLlmConnection(slug)
  ...
  const handleSetDefaultConnection = useCallback(async (slug: string) => {
    if (!window.webAgentAPI) return
    try {
      const result = await window.webAgentAPI.setDefaultLlmConnection(slug)
      if (result.success) {
        refreshLlmConnections?.()
      } else {
  ...
  const handleSetMidStreamBehavior = useCallback(async (
    ...
    try {
      ...
      const result = await window.webAgentAPI.saveLlmConnection(connectionData as import('../../../shared/types').LlmConnection)
      if (result.success) {
        refreshLlmConnections?.()
      } else {
  ...
  const handleDefaultModelChange = useCallback(async (model: string) => {
    if (!window.webAgentAPI || !defaultConnection) return
    ...
    const { isAuthenticated: _a, authError: _b, isDefault: _c, ...connectionData } = updated
    await window.webAgentAPI.saveLlmConnection(connectionData as import('../../../shared/types').LlmConnection)
  ...
  const handleDefaultThinkingChange = useCallback(async (level: ThinkingLevel) => {
    ...
    try {
      const result = await window.webAgentAPI.setDefaultThinkingLevel(level)
      if (!result.success) {
  ...
  const handleGetRtk = useCallback(() => {
    window.webAgentAPI?.openUrl('https://github.com/rtk-ai/rtk')
  }, [])

  const refreshRtkGain = useCallback(async () => {
    const gain = await window.webAgentAPI?.getRtkGain()
