...
const ChatPage = React.memo(function ChatPage({ sessionId }: ChatPageProps) {
  ...
  React.useEffect(() => {
    window.webAgentAPI.getWindowFocusState().then(setIsWindowFocused)
    const cleanup = window.webAgentAPI.onWindowFocusChange(setIsWindowFocused)
    return cleanup
  }, [])
  ...
  const handleModelChange = React.useCallback((model: string, connection?: string) => {
    if (activeWorkspaceId) {
      window.webAgentAPI.setSessionModel(sessionId, activeWorkspaceId, model, connection)
    }
  ...
  const handleConnectionChange = React.useCallback(async (connectionSlug: string) => {
    try {
      await window.webAgentAPI.sessionCommand(sessionId, { type: 'setConnection', connectionSlug })
    } catch (error) {
  ...
  const handleWorkingDirectoryChange = React.useCallback(async (path: string) => {
    if (!session) return
    await window.webAgentAPI.sessionCommand(session.id, { type: 'updateWorkingDirectory', dir: path })
  }, [session])
    ...
    async (path: string) => {
      ...
      if (resolved.startsWith('/')) {
        ...
        if (lastSlash > 0 && lastSlash < resolved.length - 1) {
          ...
          try {
            const matches = await window.webAgentAPI.searchFiles(parentDir, fileName)
  ...
  const handleShare = React.useCallback(async () => {
    const result = await window.webAgentAPI.sessionCommand(sessionId, { type: 'shareToViewer' }) as { success: boolean; url?: string; error?: string } | undefined
    if (result?.success && result.url) {
      await navigator.clipboard.writeText(result.url)
      toast.success(t('toast.linkCopied'), {
        description: result.url,
        action: { label: t('sendToWorkspace.open'), onClick: () => window.webAgentAPI.openUrl(result.url!) },
      })
    } else {
  ...
  const handleOpenInBrowser = React.useCallback(() => {
    if (sharedUrl) window.webAgentAPI.openUrl(sharedUrl)
  }, [sharedUrl])
  ...
  const handleUpdateShare = React.useCallback(async () => {
    const result = await window.webAgentAPI.sessionCommand(sessionId, { type: 'updateShare' }) as { success: boolean; error?: string } | undefined
    if (result?.success) {
  ...
  const handleRevokeShare = React.useCallback(async () => {
    const result = await window.webAgentAPI.sessionCommand(sessionId, { type: 'revokeShare' }) as { success: boolean; error?: string } | undefined
    if (result?.success) {
      ...
      <StyledDropdownMenuContent align="end" sideOffset={8}>
        {sharedUrl ? (
          <>
            ...
            <StyledDropdownMenuSeparator />
            <StyledDropdownMenuItem onClick={() => window.webAgentAPI.openUrl('https://agents.craft.do/docs/go-further/sharing')}>
