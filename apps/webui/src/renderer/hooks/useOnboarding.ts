...
export function useOnboarding({
  ...
  useEffect(() => {
    const checkGitBash = async () => {
      try {
        const status = await window.webAgentAPI.checkGitBash()
        setState(s => ({
          ...s,
  ...
  const handleSaveConfig = useCallback(async (
    ...
    try {
      ...
      // Use new unified API
      const result = await window.webAgentAPI.setupLlmConnection(
  ...
  const handleSubmitCredential = useCallback(async (data: ApiKeySubmitData) => {
    ...
    try {
      ...
      const testResult = await window.webAgentAPI.testLlmConnectionSetup({
  ...
  const saveAndValidateConnection = useCallback(async (connectionSlug: string, method: ApiSetupMethod, credential?: string, updateOnly?: boolean, oauthIdentity?: ClaudeOAuthIdentityDto): Promise<boolean> => {
    ...
    const testResult = await window.webAgentAPI.testLlmConnection(connectionSlug)
    if (testResult.success) {
      setState(s => ({ ...s, credentialStatus: 'success', step: 'complete' }))
      return true
    } else {
  ...
  const handleStartOAuth = useCallback(async (methodOverride?: ApiSetupMethod, connectionSlugOverride?: string) => {
    ...
    try {
      ...
      if (effectiveMethod === 'pi_chatgpt_oauth') {
        ...
        const result = await window.webAgentAPI.startChatGptOAuth(connectionSlug)
      ...
      if (effectiveMethod === 'pi_copilot_oauth') {
        ...
        const cleanup = window.webAgentAPI.onCopilotDeviceCode((data) => {
          setCopilotDeviceCode(data)
        })

        try {
          const result = await window.webAgentAPI.startCopilotOAuth(connectionSlug)
      ...
      const result = await window.webAgentAPI.startClaudeOAuth()
  ...
  const handleSubmitAuthCode = useCallback(async (code: string) => {
    ...
    try {
      ...
      const result = await window.webAgentAPI.exchangeClaudeCode(code.trim(), connectionSlug)
  ...
  const handleCancelOAuth = useCallback(async () => {
    ...
    await window.webAgentAPI.clearClaudeOAuthState()
  }, [])

  // Git Bash handlers (Windows only)
  const handleBrowseGitBash = useCallback(async () => {
    return window.webAgentAPI.browseForGitBash()
  }, [])

  const handleUseGitBashPath = useCallback(async (path: string) => {
    const result = await window.webAgentAPI.setGitBashPath(path)
    if (result.success) {
  ...
  const handleSkipSetup = useCallback(async () => {
    try {
      await window.webAgentAPI.deferSetup()
    } catch (error) {
  ...
  const reset = useCallback(() => {
    ...
    window.webAgentAPI.clearClaudeOAuthState().catch(() => {
