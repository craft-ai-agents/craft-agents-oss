...
export default function App() {
  ...
  useEffect(() => {
    window.webAgentAPI.isDebugMode().then((isDebug) => {
  ...
  const reconcilePermissionModeState = useCallback(async (sessionId: string) => {
    try {
      const state = await window.webAgentAPI.getSessionPermissionModeState(sessionId)
      if (!state) return
      applyPermissionModeState(sessionId, state, 'reconcile')
    } catch (error) {
      window.webAgentAPI.debugLog('[ModeSync] Failed to reconcile permission mode', {
        sessionId,
  ...
  const refreshSessionFromServer = useCallback(async (sessionId: string): Promise<'refreshed' | 'preserved_stale_messages' | 'failed'> => {
    try {
      const fresh = await window.webAgentAPI.getSessionMessages(sessionId)
  ...
  const loadSessionsFromServer = useCallback(async () => {
    ...
    try {
      const loadedSessions = await window.webAgentAPI.getSessions()
      ...
      const transportState = await window.webAgentAPI.getTransportConnectionState().catch(() => null)
  ...
  const refreshSessionListMetadataFromServer = useCallback(async (options: SessionListRefreshOptions = {}): Promise<Map<string, SessionMeta> | null> => {
    ...
    try {
      const sessions = await window.webAgentAPI.getSessions()
  ...
  const refreshLlmConnections = useCallback(async () => {
    const connections = await window.webAgentAPI.listLlmConnectionsWithStatus()
    ...
    if (windowWorkspaceId) {
      const settings = await window.webAgentAPI.getWorkspaceSettings(windowWorkspaceId)
  ...
  const handleOnboardingComplete = useCallback(async () => {
    try {
      ...
      const ws = await window.webAgentAPI.getWorkspaces()
      if (ws.length > 0) {
        // Switch to workspace in-place (no window close/reopen)
        await window.webAgentAPI.switchWorkspace(ws[0].id)
  ...
  const handleReauthLogin = useCallback(async () => {
    ...
    const needs = await window.webAgentAPI.getSetupNeeds()
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        const wsId = await window.webAgentAPI.getWindowWorkspace()
  ...
  useEffect(() => {
    ...
    window.webAgentAPI.getWorkspaces().then(setWorkspaces)
    window.webAgentAPI.getNotificationsEnabled().then(setNotificationsEnabled).catch(() => {})

    // Show actionable toast for missing system dependencies (Windows only)
    window.webAgentAPI.getSystemWarnings().then((warnings) => {
      if (warnings.vcredistMissing) {
        toast.warning(t('toast.vcRedistNotFound'), {
          ...
          action: {
            label: 'Install',
            onClick: () => window.webAgentAPI.openUrl(warnings.downloadUrl ?? 'https://aka.ms/vs/17/release/vc_redist.x64.exe'),
          },
        })
      }
    ...
    window.webAgentAPI.listLlmConnectionsWithStatus().then((connections) => {
      setLlmConnections(connections)
      setDefaultLlmConnectionSlug(resolveDefaultConnectionSlug(connections))
    })
    ...
    window.webAgentAPI.getAllDrafts().then((drafts) => {
      if (Object.keys(drafts).length > 0) {
        sessionDraftsRef.current = new Map(Object.entries(drafts))
      }
    })
    // Load app-level theme
    window.webAgentAPI.getAppTheme().then(setAppTheme)
  ...
  useEffect(() => {
    const cleanupApp = window.webAgentAPI.onAppThemeChange((theme) => {
      setAppTheme(theme)
    })
    return () => {
      cleanupApp()
  ...
  useEffect(() => {
    const cleanup = window.webAgentAPI.onLlmConnectionsChanged(() => {
  ...
  useEffect(() => {
    ...
    const cleanup = window.webAgentAPI.onSessionEvent((event: SessionEvent) => {
      ...
      if (event.type === 'session_created') {
        window.webAgentAPI.getSessionMessages(sessionId)
          .then((createdSession: Session | null) => {
            ...
            return window.webAgentAPI.getSessions().then(initializeSessions)
          })
  ...
  useEffect(() => {
    const cleanup = window.webAgentAPI.onReconnected(async (isStale: boolean) => {
      if (!isStale) {
  ...
  useEffect(() => {
    const unsubNewChat = window.webAgentAPI.onMenuNewChat(() => {
      setMenuNewChatTrigger(n => n + 1)
    })
    const unsubSettings = window.webAgentAPI.onMenuOpenSettings(() => {
      handleOpenSettings()
    })
    const unsubShortcuts = window.webAgentAPI.onMenuKeyboardShortcuts(() => {
  ...
  const handleCreateSession = useCallback(async (workspaceId: string, options?: import('../shared/types').CreateSessionOptions): Promise<Session> => {
    const session = await window.webAgentAPI.createSession(workspaceId, options)
  ...
  const handleDeleteSession = useCallback(async (sessionId: string, skipConfirmation = false): Promise<boolean> => {
    ...
    if (!skipConfirmation) {
      ...
      if (!isEmpty) {
        const confirmed = await window.webAgentAPI.showDeleteSessionConfirmation(meta?.name || 'Untitled')
        if (!confirmed) return false
      }
    }

    await window.webAgentAPI.deleteSession(sessionId)
  ...
  const handleUnarchiveSession = useCallback((sessionId: string) => {
    ...
    window.webAgentAPI.sessionCommand(sessionId, { type: 'unarchive' })
  }, [updateSessionById])
  ...
  const handleSetActiveViewingSession = useCallback((sessionId: string) => {
    ...
    window.webAgentAPI.sessionCommand(sessionId, { type: 'setActiveViewing', workspaceId: windowWorkspaceId ?? '' })
  ...
  const handleMarkSessionRead = useCallback((sessionId: string) => {
    ...
    window.webAgentAPI.sessionCommand(sessionId, { type: 'markRead' })
  }, [updateSessionById])
  ...
  const handleMarkSessionUnread = useCallback((sessionId: string) => {
    ...
    window.webAgentAPI.sessionCommand(sessionId, { type: 'markUnread' })
  }, [updateSessionById])
  ...
  const handleSessionStatusChange = useCallback((sessionId: string, state: SessionStatus) => {
    ...
    window.webAgentAPI.sessionCommand(sessionId, { type: 'setSessionStatus', state })
  }, [updateSessionById])

  const handleRenameSession = useCallback((sessionId: string, name: string) => {
    updateSessionById(sessionId, { name })
    window.webAgentAPI.sessionCommand(sessionId, { type: 'rename', name })
  }, [updateSessionById])
    ...
    try {
      ...
      if (attachments?.length) {
        ...
        const storeResults = await Promise.allSettled(
          attachments.map(a => window.webAgentAPI.storeAttachment(sessionId, a))
        )
      ...
      await window.webAgentAPI.sendMessage(sessionId, message, processedAttachments, storedAttachments, {
        skillSlugs,
        badges: badges.length > 0 ? badges : undefined,
  ...
  const handleSessionOptionsChange = useCallback((sessionId: string, updates: SessionOptionUpdates) => {
    ...
    if (updates.permissionMode !== undefined) {
      ...
      window.webAgentAPI.sessionCommand(sessionId, { type: 'setPermissionMode', mode: updates.permissionMode })
    }
    if (updates.thinkingLevel !== undefined) {
      // Sync thinking level change with backend (session-level, persisted)
      window.webAgentAPI.sessionCommand(sessionId, { type: 'setThinkingLevel', level: updates.thinkingLevel })
    }
  }, [sessionOptions])
  ...
  const hydrateDraftAttachments = useCallback(async (sessionId: string): Promise<FileAttachment[]> => {
    ...
    const results = await Promise.all(
      refs.map(async (ref) => {
        ...
        try {
          const attachment = await window.webAgentAPI.readUserAttachment(ref.path)
          if (!attachment) {
  ...
  const schedulePersistDraft = useCallback((sessionId: string) => {
    ...
    const timeout = setTimeout(() => {
      ...
      window.webAgentAPI.setDraft(sessionId, draft)
  ...
  const openNewChat = useCallback(async (params: NewChatActionParams = {}) => {
    ...
    if (params.name) {
      await window.webAgentAPI.sessionCommand(session.id, { type: 'rename', name: params.name })
    }
  ...
  const handleRespondToPermission = useCallback(async (
    ...
    const success = await window.webAgentAPI.respondToPermission(sessionId, requestId, allowed, alwaysAllow, options)
  ...
  const linkInterceptor = useLinkInterceptor({
    openFileExternal: async (path) => {
      try {
        await window.webAgentAPI.openFile(path)
      } catch (error) {
    ...
    openUrl: async (url) => {
      try {
        await window.webAgentAPI.openUrl(url)
      } catch (error) {
        const message = error instanceof Error ? error.message : 'Unknown error'
    ...
    showInFolder: async (path) => {
      try {
        await window.webAgentAPI.showInFolder(path)
      } catch (error) {
        const message = error instanceof Error ? error.message : 'Unknown error'
    ...
    readFile: (path) => window.webAgentAPI.readFile(path),
    readFileDataUrl: (path) => window.webAgentAPI.readFileDataUrl(path),
    readFileBinary: (path) => window.webAgentAPI.readFileBinary(path),
  })
  ...
  const handleReconnectTransport = useCallback(() => {
    void window.webAgentAPI.reconnectTransport().catch((error) => {
      const message = error instanceof Error ? error.message : 'Unknown error'
  ...
  const executeReset = useCallback(async () => {
    try {
      await window.webAgentAPI.logout()
      // Reset all state
  ...
  const handleSelectWorkspace = useCallback(async (workspaceId: string, openInNewWindow = false) => {
    ...
    if (openInNewWindow) {
      ...
      window.webAgentAPI.openWorkspace(workspaceId)
    } else {
      // Switch workspace in current window
      // 1. Update the main process's window-workspace mapping
      await window.webAgentAPI.switchWorkspace(workspaceId)
  ...
  const platformActions = useMemo(() => ({
    ...
    onReadFile: (path: string) => window.webAgentAPI.readFile(path),
    // Read file as data URL (used by image-preview blocks)
    onReadFileDataUrl: (path: string) => window.webAgentAPI.readFileDataUrl(path),
    // Read file as binary Uint8Array (used by PDF preview blocks)
    onReadFileBinary: (path: string) => window.webAgentAPI.readFileBinary(path),
    // Reveal a file in the system file manager (Finder on macOS, Explorer on Windows, etc.)
    onRevealInFinder: (path: string) => {
      window.webAgentAPI.showInFolder(path).catch(() => {})
    },
    ...
    onSetTrafficLightsVisible: (visible: boolean) => {
      window.webAgentAPI.setTrafficLightsVisible(visible)
    },
        ...
        <ModalProvider>
          ...
          <WorkspacePicker
            onSelectWorkspace={async (id) => {
              await window.webAgentAPI.switchWorkspace(id)
