#!/usr/bin/env bun
/**
 * Creates a GitHub issue using a sandcastle agent.
 *
 * Usage:
 *   bun .afk/create-issue.ts "description"
 *   bun .afk/create-issue.ts --agent codex "description"
 */
...
if (!description) {
    console.error("Usage: create-issue.ts [--agent claude|codex] <description>");
    process.exit(1);
}
...
await sandcastle.run({
    ...
    cwd: ROOT,
    agent,
    promptFile: join(PROMPTS, "create-issue-prompt.md"),
    promptArgs: {
    ...
    logging: {
        type: "file",
        path: ".afk/logs/create-issue.log",
    },
});
