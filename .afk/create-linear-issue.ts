#!/usr/bin/env bun
/**
 * Creates a Linear issue using a sandcastle agent.
 *
 * Usage:
 *   bun .afk/create-linear-issue.ts "description"
 *   bun .afk/create-linear-issue.ts --agent codex "description"
 */
...
if (!description) {
    console.error("Usage: create-linear-issue.ts [--agent claude|codex] <description>");
    process.exit(1);
}
...
await sandcastle.run({
    ...
    cwd: ROOT,
    agent,
    promptFile: join(PROMPTS, "create-linear-issue-prompt.md"),
    promptArgs: {
        DESCRIPTION: description,
    },
    completionSignal: "<promise>COMPLETE</promise>",
    idleTimeoutSeconds: IDLE_SECONDS,
    name: "LinearIssueCreator",
    logging: {
        type: "file",
        path: ".afk/logs/create-linear-issue.log",
    },
});
